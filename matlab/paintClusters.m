function [fig, fig2, handles]= paintClusters(clustered, template, numLevels, isRecursive, values, invcolor)
...
if isRecursive
    [handles, ~, levelShapes] = paintRecursive(fig, pax, xl, yl, axpos, ydir, clustered, template, polarPoints, [0,0], 1 / (2 * numLevels), 0, levelShapes,invcolor);
else
   [handles , levelShapes]= paintIterative(fig, pax, xl, yl, axpos, ydir, clustered, template, polarPoints,  1 / (2 * numLevels), levelShapes,values,invcolor);
end
...
function [ret, levelShapes]=paintIterative(fig, pax, xl, yl, axpos, ydir, clustered, template,polarPoints, offsetR, levelShapes, values,invcolor)
cnt = 1;
...
for c=1:size(clustered,1)
    ...
    if level == 0
        ...
        ret{cnt}.handle = paintSingle(fig, pInds, template, ...
            convertToFigureSpace(axpos, ydir, xl, yl, [0.5,0.5] + center), offsetR, nan, value,invcolor);
        levelShapes{level + 1} = paintSingle(nan, pInds, template, nan, nan, levelShapes{level + 1},value,invcolor);
        ret{cnt}.level = level;
        ret{cnt}.id = 1;
        ret{cnt}.parent = 0;
        cnt = cnt + 1;
...
function [ret, polarPoints, levelShapes] = paintRecursive(fig, pax, xl, yl, axpos, ydir, clustered, template,polarPoints, center,  offsetR, level, levelShapes, invcolor)
if level > 0
    levelPolarPoints = polarPoints{level};
...
ret.handle= paintSingle(fig, clustered.rootIndices, template, convertToFigureSpace(axpos, ydir, xl, yl, [0.5,0.5] + newCenter), ...
    offsetR, nan, nan,invcolor);
levelShapes{level + 1} = paintSingle(nan, clustered.rootIndices, template, nan, nan, levelShapes{level + 1},nan,invcolor);
...
function ax =paintSingle(fig, rootIndices, template,  center, axisSize, shape, value,invcolor)
...
if isa(fig,'matlab.ui.Figure')
    ax = showPaintedDoubleFace(fig, shape, center,axisSize,invcolor);
else
    ax = shape;
end
end

function [fig, fig2, handles]= paintClusters(clustered, template, numLevels, isRecursive, values)
...
if isRecursive
   ...
   [handles , levelShapes]= paintIterative(fig, pax, xl, yl, axpos, ydir, clustered, template, polarPoints,  1 / (2 * numLevels), levelShapes,values);
end
...
for i =1:(numLevels + 1)
    shape = levelShapes{i};    
    ax = showPaintedDoubleFace(fig2, shape, nan, nan, nexttile(t), ~isempty(values),[minValue,maxValue]);
    daspect(ax, [1 1 1]);
end
if ~isempty(values)
...
function [ret, levelShapes]=paintIterative(fig, pax, xl, yl, axpos, ydir, clustered, template,polarPoints, offsetR, levelShapes, values)
cnt = 1;
...
for c=1:size(clustered,1)
    ...
    if level == 0
        ...
        ret{cnt}.handle = paintSingle(fig, pInds, template, ...
            convertToFigureSpace(axpos, ydir, xl, yl, [0.5,0.5] + center), offsetR, nan, value);
...
function ax =paintSingle(fig, rootIndices, template,  center, axisSize, shape, value)
...
if isa(fig,'matlab.ui.Figure')
    ax = showPaintedDoubleFace(fig, shape, center,axisSize);
else
    ax = shape;
end
end

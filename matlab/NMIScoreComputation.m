...
input_mat_path = @(component)sprintf('../results/hierarchicalClusteringDemo/%s/%s_reduction%d/levels%d/input.mat', ...
    DATASET,component,1/REDUCTION,LEVELS);
...
level = level';
writetable(array2table(NMI_score, 'RowNames', string(level), 'VariableNames',  {'Asymmetry-Atlas','Symmetry-Atlas','Asymmetry-Symmetry'}), ['../results/hierarchicalClusteringDemo/' DATASET '/asymmetryVsSymmetryNMI' num2str(1/REDUCTION)]);
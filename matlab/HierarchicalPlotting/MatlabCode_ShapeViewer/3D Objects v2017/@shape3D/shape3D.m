classdef shape3D < superHandleClass
    ...
    properties (Hidden = true, Transient = true)
        ...
        PatchClickedPoint;% Information when patch was clicked on;
    end
    ...
    methods % SIMPLE INTERFACING
        ...
        function out = geometricImage(obj,res,range)
                if obj.Level<2, out = []; return; end% A Mesh is required
                if isempty(obj.UV), out = []; return; end%  A valid UV mapping is to be given
                if nargin<3
                ...
                warning('off','All');
                for i=1:1:3
                    im(:,:,i) = griddata(UV(1,:),UV(2,:),obj.Vertices(:,i)',X,Y,'cubic'); %#ok<GRIDD>
    ...
    methods (Static = true) % STATIC functions
        ...
        function myClick(varargin)
            % return the point on the surface when clicked ('EXTENT BUTTON; middle mouse button')upon
            if ~(varargin{2}.Button==2), return; end
            obj = varargin{1}.UserData;
            obj.PatchClickedPoint = varargin{2}.IntersectionPoint;
            %disp(obj.PatchClickedPoint);
        end
    end
end
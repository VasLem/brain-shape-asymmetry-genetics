...
function KeyPress(varargin)
 ...
 if ~obj.KeyReleased, return;end
 obj.KeyReleased = false;
...
function otherKeys(input,obj)
       switch input.Key
           ...
           case 'r'% remove area in brush mode
               if strcmp(obj.SelectionMode,'brush')&&obj.SelectionActive
                  areaindex = select3DRadiusArea(obj);
...
function BtnDown(varargin)
     obj = get(varargin{1},'UserData');
     if obj.MuteCallbacks, return;end
     if strcmp(get(obj.Figure, 'selectiontype'),'extend'), return; end% extent is reserved for select3D point
     if strcmp(obj.SelectionMode,'brush')&&strcmp(get(obj.Figure, 'selectiontype'),'open'), return; end
     ...
     switch obj.SelectionMode
        case 'landmark'
                 ...
                 switch get(obj.Figure, 'selectiontype')
                     ...
                     case 'alt'
                         p = mySelect3DPoint(obj);if isempty(p), return; end   
                         updateLMSelection(obj,'Delete LM',p);
                     case 'extend' %reserved
...
function BtnUp(varargin)
                 ...
                 if obj.SelectionActive
                    ...
                    if ~PointerOnFigure(obj.Figure,scrn_pt)
                       ...
                       for f=1:1:length(figure_list)
                           ...
                           if ~isa(UD,obj.Type), continue; end
                           if (UD==obj), break; end
                           selection = get(obj.Figure, 'CurrentObject');
                           shapeobject = get(selection,'UserData');
         ...
         if~obj.SelectionActive, set(obj.Figure,'WindowButtonMotionFcn',[]); obj.Action = 'none';end 
end

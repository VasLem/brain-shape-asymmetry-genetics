function out = solveRecord(obj,varargin)
         ...
         % if FS == Visible, set Show == 1
           ...
           obj.ObjFun.Target.Visible = true;
         % Keep a handle copy of the original Floating surface
           ...
           obj.ObjFun.Target.ColorMode = 'Single';
%            obj.ObjFun.Target.SingleColor = [0 0 0];
%            obj.ObjFun.Target.Border.Visible = false;
%            obj.ObjFun.Target.PoseLM.Visible = false;
         % Start the Clock
           tic;
          
           vrecord = get(obj.ObjFun.Target.Axes,'UserData');
           vrecord.SceneLightLinked = true;
           vrecord.SceneLightVisible = true;
           obj.ObjFun.Target.Axes = vrecord.RenderAxes;
           pause;
           obj.ObjFun.Target.SingleColor = [0.5 0.5 0.5];
           obj.ObjFun.Target.Border.Visible = false;
           obj.ObjFun.Target.PoseLM.Visible = false;
           
           set(vrecord.Toolbar.video,'State','on');
           pause(0.0001);
           %captureFrame(vrecord);
         % Initialization
           initialize(obj);        
           pause(0.0001);
         ...
         % Finalize
           ...
           pause(0.0001)
           captureFrame(vrecord);
           set(vrecord.Toolbar.video,'State','off');
         % take care of output;   

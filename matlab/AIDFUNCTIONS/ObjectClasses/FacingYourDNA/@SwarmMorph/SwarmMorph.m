classdef SwarmMorph < superClass
    ...
    methods % CONSTRUCTOR
        function obj = SwarmMorph(varargin)
    ...
    methods % AID INTERFACE FUNCTIONS
        function updateGBest(obj)
            [val,ind] = min(obj.Scores);
            if val>=obj.gBestScore, return; end
            obj.gBest = obj.Pop(ind,:);
            obj.gBestScore = val;
        end
        function updatePBest(obj)
            tmp = obj.Scores<obj.pBestScore;
            obj.pBest(find(tmp),:) = obj.Pop(find(tmp),:);
            obj.pBestScore(find(tmp)) = obj.Scores(find(tmp));
        end
        function updatePVel(obj)
%             obj.pVel = obj.pVel+...
%                        obj.pIncrement.*randn(obj.PopSize,obj.Dim).*(obj.pBest-obj.Pop)+...
%                        obj.gIncrement.*randn(obj.PopSize,obj.Dim).*(repmat(obj.gBest,obj.PopSize,1)-obj.Pop);
            obj.pVel = obj.pVel+...
                       obj.pIncrement.*rand(obj.PopSize,obj.Dim).*(obj.pBest-obj.Pop)+...
                       obj.gIncrement.*rand(obj.PopSize,obj.Dim).*(repmat(obj.gBest,obj.PopSize,1)-obj.Pop);

        end
        function updatePos(obj)
           obj.Pop = obj.Pop+obj.pVel; 
        end
        ...
        function InitializeVel(obj)
            R = obj.GlobalRange/2;
            %obj.pVel = (repmat(R(:,1),1,obj.PopSize) + repmat(R(:,2)-R(:,1),1,obj.PopSize).*randn(obj.Dim,obj.PopSize))';
            obj.pVel = rand(obj.PopSize,obj.Dim);
        end
    end

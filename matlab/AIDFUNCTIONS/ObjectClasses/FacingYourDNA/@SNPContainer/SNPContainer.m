classdef SNPContainer < superClass
   ...
   methods % Interface functions
       ...
       function runStage(obj,stage,BaseCont,t,varargin)
           ...
           parfor i=1:nrSNP
               switch stage
                   ...
                   case 2
                       done{i} = runStage2(snps{i},inputdata.DepVar,parcont);
                   case 3
                       done{i} = runStage3(snps{i},inputdata.DepVar,inputdata.Cov,parcont);
                   case 4
                       done{i} = runStage4(snps{i},inputdata.BaseCont,inputdata.UseRedShape,parcont);
               end
               parfor_progress;
           end
       ...
       function prepareStage3Splitting(obj)
           ...
           for i=1:1:obj.nrSNP
               ...
               for j=1:1:length(testindex)
                  % survival either F statistic or G statistic 
                  if forsnp.ST2FCode(testindex(j)) || forsnp.ST2GCode(testindex(j)) 
                     counter = counter + 1;
                     tmp = clone(forsnp);
       ...
       function prepareStage3(obj,tests,stages)
           f = waitbar(0,'Checking');drawnow;
           if nargin<3, tests = (0:10); end
           TestInd = getValues(obj,'ST2FTestInd');
           nrSNP = length(find(ismember(TestInd(1,:),tests)));
           counter = 0;
           for i=1:length(tests)
              index = find(TestInd(1,:)==tests(i));
              if isempty(index), continue; end
              disp('-----------------------------------------------------------------------------------------------');
              disp('    ');
              switch tests(i)
                  case 0
                      disp('CHECKING NO EFFECT CANDIDATES');
                  case 1
                      disp('CHECKING RECESSIVE CANDIDATES');
                  case 2
                      disp('CHECKING DOMINANT CANDIDATES');
                  case 3
                      disp('CHECKING OVER DOMINANT CANDIDATES');
                  case 4
                      disp('CHECKING ADDITIVE CANDIDATES');
                  case 5
                      disp('CHECKING AA BB CANDIDATES');
                  case 6
                      disp('CHECKING AA AB CANDIDATES');
                  case 7
                      disp('CHECKING AB BB CANDIDATES');
                  case 8
                      disp('CHECKING NON ADDITIVE CANDIDATES');    
                  case 9
                      disp('CHECKING AA BB AB CANDIDATES');
                  case 10
                      disp('CHECKING AB AA BB CANDIDATES');
              end
              disp('    ');
              disp(['Number to check: ' num2str(length(index))]);
              disp('-----------------------------------------------------------------------------------------------');
              for j=1:1:length(index)
                  disp(['Candidate: ' num2str(j) '/' num2str(length(index)) '    General Index: ' num2str(index(j))]);
                  disp('    ');
                  prepareStage3(obj.SNPs{index(j)},stages);
                  counter = counter + 1;
                  disp('-----------------------------------------------------------------------------------------------');
                  waitbar(counter/nrSNP,f);drawnow;
              end
           end
           delete(f);drawnow;
           disp('REMOVING FALSE SNPs');
           tmp = getValues(obj,'ST3TestInd');reduceSNPs(obj,find(tmp(1,:)));
           disp('DONE');
       end
       ...
       function splitNonAdditiveSNPs(obj)
           out = getValues(obj,'ST3TestInd');
           index = find(out(1,:)==8);
           if isempty(index), return; end
           new = cell(1,length(index)*3);
           counter = 0;
           disp(['Splitting ' num2str(length(index)) ' SNP(s)']);
           for i=1:1:length(index)
               for j=1:1:3 % each SNP is copied 3X
                  counter = counter + 1; 
                  tmp = clone(obj.SNPs{index(i)});
                  switch j
                      case 1 % RECESSIVE PART
                          tmp.ST3TestInd = 1;
                      case 2 % DOMINANT PART 
                          tmp.ST3TestInd = 2;
                      case 3 % OVER DOMINANT PART
                          tmp.ST3TestInd = 3;
                  end
                  new{counter} = tmp;
               end
           end
           reduceSNPs(obj,setdiff(1:obj.nrSNP,index));% remove the old
           addSNPs(obj,new);% add the new
       end
       ...
       function out = getGTAtyp(obj,genotypes)
                ...
                for i=1:1:obj.nrSNP
                    ...
                    out(i) = obj.SNPs{i}.ST4MOD.Atyps(ind);
                end
       end

classdef BASEMOD < superClass
   ...
   methods % COVARIATE ANALYSES
       function out = runR2TestCOV(obj,DepVar,GBindex,t)
           ...
           parfor i=1:1:obj.nCOV
               switch obj.COVMultiple
                   case false
                       [R2(i),pR2(i)] = BASEMOD.testPartialPLSR(obj.COV(:,i),DepVar,GB,t);
                   case true
                       rm = setdiff(all,i);
                       [R2(i),pR2(i)] = BASEMOD.testPartialPLSR(obj.COV(:,i),DepVar,[obj.COV(:,rm) GB],t);
               end
               parfor_progress;
           end
       ...
       function out = runAngleTestCOV(obj,DepVar,t,maxM,Aval,GBindex,AngleTable)
           DepVar = getDepVar(obj,DepVar);
           ShapeDim = size(DepVar,2);
           M = nan*zeros(ShapeDim,2,maxM,obj.nCOV);
           ...
           parfor a=1:obj.nCOV
               diff = setdiff(all,a);
               [tmpA,tmpM,STAT] = BASEMOD.AngleTest([obj.COV(:,diff) GB obj.COV(:,a)],DepVar,Aval,maxM,t);
           ...
           out.pRA = BASEMOD.lookUppRA(out.AvgA,size(DepVar,2),AngleTable);
           out.Aval = Aval;
           obj.COVATest = rmfield(out,{'A' 'M'});
       end
       function buildFromAngleTestCOV(obj,DepVar,test)
           DepVar = getDepVar(obj,DepVar);
           out = cell(1,obj.nCOV);
           for i=1:obj.nCOV
             ...
             % VARIABLE INFORMATION  
               ...
               out{i}.Var.Info{1} = BASEMOD.getVarInfo(out{i}.IndVar,DepVar);
               fullM = [squeeze(forM(:,1,:)) squeeze(forM(:,2,:))];
               out{i}.M = nanmedian(fullM,2)';
               out{i}.H = BASEMOD.wilcoxonMSimple(fullM',obj.HTestp);
               out{i}.HM = out{i}.M.*out{i}.H;
              % RIP COMPUTATIONS
               out{i}.MRIP = BASEMOD.updateRIP(DepVar,out{i}.M)';
               out{i}.NormMRIP = BASEMOD.normalizeRIP(out{i}.MRIP,out{i}.M,out{i}.Var);
               out{i}.HMRIP = BASEMOD.updateRIP(DepVar,out{i}.HM)';
               out{i}.NormHMRIP = BASEMOD.normalizeRIP(out{i}.HMRIP,out{i}.HM,out{i}.Var);
           end
           obj.COVMOD = out;
       end
       function out = biometricTestCOV(obj,TestDepVar,TestCOV,nrF)
            ...
            parfor i=1:obj.nCOV
                MOD = obj.COVMOD{i};
                testcov = TestCOV(:,i);
                RIP = BASEMOD.updateRIP(TestDepVar,getMODM(obj,MOD))';
                if obj.RIPNorm, RIP = BASEMOD.normalizeRIP(RIP,getMODM(obj,MOD),MOD.Var);end
                ...
                for t=1:nrT
                    ...
                    Distr = BASEMOD.X2RIP(MOD.IndVar,getMODRIP(obj,MOD),testcov(t),MOD.Var.Info{1}.Type,obj.Kappa,obj.KN);
                ...
                switch obj.Classify
                   ...
                   case 'HARD'
                       TScores = BASEMOD.class2score(match2class(obj,TMatches));
                       FScores = BASEMOD.class2score(match2class(obj,FMatches));
                end
                ClassACC(i) = ((sum(match2class(obj,TMatches),2)/nrT)*100);
                [forRanks,EER(i),~,~,~,~,AUC(i)] = BASEMOD.getBiometrics(TScores,FScores,ones(1,nrT),ones(size(ones(1,nrT),1),1));
   ...
   methods % GENETIC BACKGROUND ANALYSES
       function out = runR2TestGB(obj,DepVar,t)
           DepVar = getDepVar(obj,DepVar);
           ...
           parfor i=1:1:obj.nGB
               switch obj.GBMultiple
                   case false
                       [R2(i),pR2(i)] = BASEMOD.testPartialPLSR(obj.GB(:,i),DepVar,obj.COV,t);
                   case true
                       rm = setdiff(all,i);
                       [R2(i),pR2(i)] = BASEMOD.testPartialPLSR(obj.GB(:,i),DepVar,[obj.COV obj.GB(:,rm)],t);
               end
               parfor_progress;
           end
           closeParForProgress(path,ID);
       ...
       function out = runAngleTestGB(obj,DepVar,t,maxM,Aval,AngleTable)
           if nargin<5, Aval = 0; end
           if nargin<4, maxM = t; end
           DepVar = getDepVar(obj,DepVar);
           M = nan*zeros(size(DepVar,2),2,maxM,obj.nGB);
           ...
           [path,ID] = setupParForProgress(obj.nGB);
           parfor a=1:obj.nGB
               [tmpA,tmpM,STAT] = BASEMOD.AngleTest([obj.COV obj.GB(:,a)],DepVar,Aval,maxM,t);
               A(:,a) = tmpA;M(:,:,:,a) = tmpM;pA(:,a) = STAT.pA;avgA(a) = STAT.avgA;
               medA(a) = STAT.medA;stdA(a) = STAT.stdA;madA(a) = STAT.madA;
               upperciA(a) = STAT.upperciA;lowerciA(a) = STAT.lowerciA;
               parfor_progress;
           ...
           out.pRA = BASEMOD.lookUppRA(out.AvgA,size(DepVar,2),AngleTable);
           out.Aval = Aval;
           obj.GBATest = rmfield(out,{'A' 'M'});
       end
       function buildFromAngleTestGB(obj,DepVar,test)
           DepVar = getDepVar(obj,DepVar);
           out = cell(1,obj.nGB);
           for i=1:obj.nGB
       ...
       function out = biometricTestGB(obj,TestDepVar,TestGB,nrF)
            ...
            parfor i=1:obj.nGB
                ...
                for t=1:nrT
                    ...
                    [~,Find] = sort(diff);
                    Find = Find(end-nrF+1:end);
                    Distr = BASEMOD.X2RIP(MOD.IndVar,getMODRIP(obj,MOD),testgb(t),'Continuous',obj.Kappa,obj.KN);
                    m = getMatch(obj,Distr,RIP);
                    TMatches(1,t) = m(t);
                    FMatches(1,t,:) = m(Find);
                end
   ...
   methods % INTERFACING FUNCTIONS
       ...
       function out = PLSR(obj,DepVar,IndVar,BootInd,CondInd)                  
                ...
                % needed to rescale RIP variables onto common scales
                  for i=1:1:var.nr2Boot
                     ...
                     var.Info{i} = BASEMOD.getVarInfo(v,DepVar);    
                  end
                  out.Var = var;
                  % getting the final M from outer training data
                  [M,H] = BASEMOD.getBootSampleRegression(IndVar,DepVar,var,obj.RegRuns,obj.SamplePerc,obj.Htestp);
                  if obj.Htest, M = H.*M;end
                  out.M = M;out.H = H;
                  RIP = BASEMOD.updateRIP(DepVar,M)';
                  if obj.RIPNorm, RIP = BASEMOD.normalizeRIP(RIP,M,var);end
                  out.RIP = RIP;
                  [out.RIPStat] = BASEMOD.performRIPStat(out.IndVar,RIP,var,0);
       end
                ...
                % needed to rescale RIP variables onto common scales
                  ...
                  while ContBoot && Bootstrap>0
                           ...
                           for fi=1:obj.InnerFold
                                ...
                                [M,H] = BASEMOD.getBootSampleRegression(IndVar(FiTrInd,:),DepVar(FiTrInd,:),var,obj.RegRuns,obj.SamplePercentage,obj.HtestP);
                                if obj.Htest, M = H.*M;end% setting close to zero partial regression coefficients to zero
                                rip = BASEMOD.updateRIP(DepVar(FiTestInd,:),M)';
                                if obj.RIPNorm, rip = BASEMOD.normalizeRIP(rip,M,var);end
                  ...
                  % getting the final M from outer training data
                  [M,H] = BASEMOD.getBootSampleRegression(IndVar,DepVar,var,obj.RegRuns,obj.SamplePercentage,obj.HtestP);
                  if obj.Htest, M = H.*M;end
                  out.M = M;out.H = H;
                  RIP = BASEMOD.updateRIP(DepVar,M)';
                  if obj.RIPNorm, RIP = BASEMOD.normalizeRIP(RIP,M,var);end
       ...
       function out = getPartitionsCOVR2(obj,F)
           ...
           parfor i=1:t
              ...
              for k=1:K
                  ind = find(F(:,i)==k);
                  [~,tmp(k)] = BASEMOD.getMR2(obj.COV(ind,:),obj.DepVar(ind,:));
              end
              out(:,i) = tmp;
           end
       end
   ...
   methods % BIOMETRICS
       function out = biometrics(obj,TestDepVar,TestCOV,TestGB,indCOV,short,vis,AngleTable)
            L1CL1TestDepVar = BASEMOD.getL1CL1DepVar(TestDepVar);
            TestDepVar = getDepVar(obj,TestDepVar);
            dim = size(TestDepVar,2);
           % TO BE UPDATED
           % Initialize
            nrT = size(TestDepVar,1);
           % extracting COV RIP values
            ...
            wCOVPR = nan*zeros(obj.nCOV,1);
            for i=1:1:obj.nCOV
                tmp = BASEMOD.updateRIP(TestDepVar,getMODM(obj,obj.COVMOD{i}))';
                if obj.RIPNorm, tmp = BASEMOD.normalizeRIP(tmp,getMODM(obj,obj.COVMOD{i}),obj.COVMOD{i}.Var);end
                RIPCOV(:,i) = tmp;
                wCOVA(i) = max(obj.COVATest.AvgA(i),0);
                wCOVP(i) = BASEMOD.pAWeight(obj.COVATest.pA(1,i),0.05);
                wCOVPR(i) = BASEMOD.pRAWeight(obj.COVATest.AvgA(i),dim,AngleTable);
           ...
           % extracting GB RIP values
            ...
            wGBPR = nan*zeros(obj.nCOV,1);
            for i=1:1:obj.nGB
                tmp = BASEMOD.updateRIP(TestDepVar,getMODM(obj,obj.GBMOD{i}))';
                if obj.RIPNorm, tmp = BASEMOD.normalizeRIP(tmp,getMODM(obj,obj.GBMOD{i}),obj.GBMOD{i}.Var);end
                RIPGB(:,i) = tmp;
                wGBA(i) = max(obj.GBATest.AvgA(i),0);
                wGBP(i) = -log(obj.GBATest.pA(1,i));
                wGBPR(i) = BASEMOD.pRAWeight(obj.GBATest.AvgA(i),dim,AngleTable);
           ...
           % extracting matching scores 
            ...
            parfor t=1:nrT
                ...
                for i=1:1:obj.nGB
                    Distr = BASEMOD.X2RIP(obj.GBMOD{i}.IndVar,getMODRIP(obj,obj.GBMOD{i}),TestGB(t,i),obj.GBMOD{i}.Var.Info{1}.Type,obj.Kappa,obj.KN);
                ...
                for i=1:1:obj.nCOV
                    Distr = BASEMOD.X2RIP(obj.COVMOD{i}.IndVar,getMODRIP(obj,obj.COVMOD{i}),TestCOV(t,i),obj.COVMOD{i}.Var.Info{1}.Type,obj.Kappa,obj.KN);
            ...
            switch obj.Classify
               ...
               case 'HARD'
                   TScoresGB = BASEMOD.class2score(match2class(obj,TMatchesGB));out.TScoresGB = TScoresGB;
                   FScoresGB = BASEMOD.class2score(match2class(obj,FMatchesGB));out.FScoresGB = FScoresGB;
                   TScoresCOV = BASEMOD.class2score(match2class(obj,TMatchesCOV));out.TScoresCOV = TScoresCOV;
                   FScoresCOV = BASEMOD.class2score(match2class(obj,FMatchesCOV));out.FScoresCOV = FScoresCOV;
            end 
            out.ClassAccGB = ((sum(match2class(obj,TMatchesGB),2)/nrT)*100)';
            out.ClassAccCOV = ((sum(match2class(obj,TMatchesCOV),2)/nrT)*100)';
            out.wCOVA = wCOVA;out.wCOVP = wCOVP;out.wCOVPR = wCOVPR;
            out.wGBA = wGBA;out.wGBP = wGBP;out.wGBPR = wGBPR;
            out.wCOVF = ones(obj.nCOV,nrT);
            out.wCOVF(isnan(out.TScoresCOV)) = nan;
            out.wGBF = ones(obj.nGB,nrT);
            out.wCOVGB(isnan(out.TScoresGB)) = nan;
            if short, return; end
            ...
            [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BASEMOD.getBiometrics(TScoresCOV(indCOV,:),FScoresCOV(indCOV,:,:),ones(nCOV,nrT),ones(nCOV,1));
            %out1 = BASEMOD.evalSortedResults(ST,SF,TestDepVar,T);
            r=r+1;% COVARIATES WEIGHTED
            [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BASEMOD.getBiometrics(TScoresCOV(indCOV,:),FScoresCOV(indCOV,:,:),ones(nCOV,nrT),wCOV);
%             out1 = BASEMOD.evalSortedResults(ST,SF,L1CL1TestDepVar,T);
            r=r+1;% GENETIC BACKGROUND UNWEIGHTED
            [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BASEMOD.getBiometrics(TScoresGB,FScoresGB,ones(obj.nGB,nrT),ones(obj.nGB,1));
            %out1 = BASEMOD.evalSortedResults(ST,SF,TestDepVar,T);
            r=r+1;% GENETIC BACKGROUND WEIGHTED
            [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BASEMOD.getBiometrics(TScoresGB,FScoresGB,ones(obj.nGB,nrT),wGB);
%             out1 = BASEMOD.evalSortedResults(ST,SF,L1CL1TestDepVar,T);
            r=r+1;% COMBINED UNWEIGHTED
            [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BASEMOD.getBiometrics([TScoresCOV(indCOV,:); TScoresGB],[FScoresCOV(indCOV,:,:); FScoresGB],ones(obj.nGB+nCOV,nrT),ones(nCOV+obj.nGB,1));
            %out1 = BASEMOD.evalSortedResults(ST,SF,TestDepVar,T);
             r=r+1;% COMBINED WEIGHTED
            [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BASEMOD.getBiometrics([TScoresCOV(indCOV,:); TScoresGB],[FScoresCOV(indCOV,:,:); FScoresGB],ones(obj.nGB+nCOV,nrT),[wCOV;wGB]);
%             out1 = BASEMOD.evalSortedResults(ST,SF,L1CL1TestDepVar,T);
       ...
       function out = match2class(obj,matches)
          ...
          out = BASEMOD.matchthresholding(matches,T);
       end
   end
   methods (Static = true)
       function [R2,pR2] = testPartialPLSR(X,Y,C,t)
           index = intersect(BASEMOD.notNAN(C),BASEMOD.notNAN(X));
           if ~isempty(C)
            E = BASEMOD.getResiduals(C(index,:),Y(index,:));
            X = BASEMOD.getResiduals(C(index,:),X(index,:));
           else
            E = Y(index,:);
            X = X(index,:);
           end
       ...
       function [A,M,STAT] = AngleTest(IndVar,DepVar,Aval,maxM,t)
               A = nan*zeros(t,1);
               ShapeDim = size(DepVar,2);
               ...
               for i=1:t
                   ...
                   for k=1:1:2
                       ind = find(F==k);
                       forM(:,k) = BASEMOD.getRegression(IndVar(ind,:),DepVar(ind,:),vid);
                   end
       ...
       function [M,H] = getBootSampleRegression(IndVar,DepVar,var,runs,perc,Tp)
                ...
                for s=1:1:runs
                    ...
                    M(:,:,s) = BASEMOD.getRegression(IndVar(SampleFi,:),DepVar(SampleFi,:),var.Booting);
                end
                if runs==1,M = squeeze(M);H = ones(size(M));return;end
                H = BASEMOD.wilcoxonM(M,var,Tp);
                M = median(M,3);       
       end
       ...
       function [Distr,RIP] = X2RIP(trX,trrip,X,type,kappa,K)
           ...
           switch type
               case 'Categorical'
                  ...
                  [Distr.Tmu,Distr.Tsigma,Distr.TO] = BASEMOD.extractRobustRIPStatistics(trrip(Tind)',kappa);
                  RIP = Distr.Tmu;Distr.TpMax = normpdf(Distr.Tmu,Distr.Tmu,Distr.Tsigma);
                  [Distr.Fmu,Distr.Fsigma,Distr.FO] = BASEMOD.extractRobustRIPStatistics(trrip(Find)',kappa);
               ...
               case 'Continuous'
                  ...
                  [Distr.Tmu,Distr.Tsigma,Distr.TO] = BASEMOD.extractRobustRIPStatistics(rip,kappa,WT);
                  ...
                  [Distr.Fmu,Distr.Fsigma,Distr.FO] = BASEMOD.extractRobustRIPStatistics(rip,kappa,WF);
                  Distr.FpMax = normpdf(Distr.Fmu,Distr.Fmu,Distr.Fsigma); 
           end
       end
                ...
                for s=1:1:length(var.Booting)
                   ...
                   switch var.Info{s}.Type
                       case 'Categorical'
                           ...
                           [stat(s),~,statP(s)] = BASEMOD.myPermAnova(XG,vrip(index),t);
                       case 'Continuous'
                           statType{s} = 'Cor';
                           [stat(s),statP(s)] = BASEMOD.permCorr(v(index),vrip(index),t);
                   end
                end  
       end
       ...
       function [R,EER,ST,SF,X,Y,AUC,T] = getBiometrics(TScores,FScores,wM,W)
                 ...
                 ST = BASEMOD.accScores(TScores,tmpW)';
                 SF = BASEMOD.accScores(FScores,repmat(tmpW,1,1,nF));
                 [R,EER,X,Y,AUC,T] = BASEMOD.evalScores(ST,SF,nT,nF); 
       end
...
%                    for k=1:1:K
%                        ind = find(F==k);
%                        forM(:,k) = BASEMOD.getRegression([obj.COV(ind,:) obj.GB(ind,a)],obj.DepVar(ind,:),vid);
%                    end
...
%                   SInd = find(Ffor==f);
%                   for a=1:obj.nGB
%                     [tmpR2(f,a),~,M(f,a,:)] = BASEMOD.testPartialPLSR(obj.GB(SInd,a),obj.DepVar(SInd,:),obj.COV(SInd,:),0);
%                   end
%                end

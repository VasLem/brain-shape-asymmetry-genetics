...
%        end
       ...
       function out = get.GGGTBal(obj)
          if isempty(obj.GT), out = []; return; end
          out = SNPMV3.getGGGTBalance(obj.GG,obj.GT);
       end
       ...
       function out = get.CALLRATE(obj)
            out = 1-(sum(isnan(obj.X))/obj.nS);
       end
       function out = get.MAlF(obj)
       ...
       function out = get.fmm(obj)
                out = (obj.nmm/(obj.nmm+obj.nMm+obj.nMM));
       end
       function out = get.fMm(obj)
                out = (obj.nMm/(obj.nmm+obj.nMm+obj.nMM));
       end
   ...
   methods % INTERFACING
       ...
       function out = getXREG(obj,in)
                out = GT2GG(obj,in);
       end
       function out = monoCheck(obj)
                tmp = obj.GT(~isnan(obj.GT));
   ...
   methods % BIOMETRICS
      ...
      function out = getGTFreq(obj,in)
               if isnan(in), out = nan; return; end
               in = in+1;
               switch in
                   case 0
                       out = (1-obj.mAlF)*(1-obj.mAlF);
                   case 1
                       out = (obj.mAlF)*(1-obj.mAlF);
                   case 2
                       out = (obj.mAlF)*(obj.mAlF);
               end
      end
   end
   methods (Static = true)
       ...
       function out = initGT(in)
           out = double(in);
%            if sum(out==2)>0 % conversion from 0-1-2 to -1-0-1
%               out(in==-1) = nan;
%               out(in==0) = -1;
%               out(in==1) = 0;
%               out(in==2) = 1;

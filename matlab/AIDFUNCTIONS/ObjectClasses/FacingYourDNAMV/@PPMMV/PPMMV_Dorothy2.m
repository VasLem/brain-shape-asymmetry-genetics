...
% OBJECT METHODS    
    ...
    methods % ASSOCIATION TESTING
       ...
       function out = runPropagatingR2Test(obj,DepVar,COV,t)        % created by Dorothy
            ...
            L1CL1DepVar =  PPMMV.getL1CL1DepVar(DepVar);
            nS = size(L1CL1DepVar,1);
            forcedPerms = PPMMV.getForcedPermutations(t,nS);
            ...
            parfor cl = 1:1:2^(l-1),
                ...
                [tmpppR2(1,cl), tmpppR2Perms(:,cl)] = PPMMV.testPartialPLSR_Permutated(obj.XREG,getDepVar(obj,DepVar,tmpclind),getCOV(obj,COV,tmpclind),t,forcedPerms);
                clind(cl) = tmpclind;
            end
            ...
            for l = obj.nLev-1:-1:1,
                ...
                parfor cl = 1:1:2^(l-1),
                    ...
                    tmpclind = getClind(obj,l,cl);
                    [ppR2_temp, ppR2Perms_temp] = PPMMV.testPartialPLSR_Permutated(obj.XREG,getDepVar(obj,DepVar,tmpclind),getCOV(obj,COV,tmpclind),t,forcedPerms);
                    clind(cl) = tmpclind;
                    ...
                    %[ppR2(clind), ppR2Perms(:,clind)] = PPMMV.nonParametricCombination ([ppR2_temp,ppR2(clind1),ppR2(clind2)], [ppR2Perms_temp,ppR2Perms(:,clind1),ppR2Perms(:,clind2)]); 
                    [tmpppR2(1,cl), tmpppR2Perms(:,cl)] = PPMMV.nonParametricCombination ([ppR2_temp,ppR2(clind1),ppR2(clind2)], [ppR2Perms_temp,ppR2Perms(:,clind1),ppR2Perms(:,clind2)]); 
                end
                ppR2(1,clind) = tmpppR2;
                ppR2Perms(:,clind) = tmpppR2Perms;
            end
       ...
       function out = runPropagatingAngleTest(obj,DepVar,COV,maxM,AngleTable,t,t2)    % created by Dorothy (long computation time)
            ...
            L1CL1DepVar =  PPMMV.getL1CL1DepVar(DepVar);
            nS = size(L1CL1DepVar,1);
            forcedPerms = PPMMV.getForcedPermutations(t2,nS);
            ...
            parfor cl = 1:1:2^(l-1),
                tmpclind = getClind(obj,l,cl);
                %[ppA(clind), ppAPerms(:,clind), ppAR(clind), ppARPerms(:,clind)] = AngleTest_Permutated([getCOV(obj,COV,tmpclind) obj.XREG],getDepVar(obj,DepVar,tmpclind),AngleTable,t,maxM,0,t2,forcedPerms);
                [tmpppA(cl), tmpppAPerms(:,cl), tmpppAR(cl), tmpppARPerms(:,cl)] = PPMMV.AngleTest_Permutated([getCOV(obj,COV,tmpclind) obj.XREG],getDepVar(obj,DepVar,tmpclind),AngleTable,t,maxM,0,t2,forcedPerms);
                clind(cl) = tmpclind;
            end
            ppA(1,clind) = tmpppA;
            ...
            for l = obj.nLev-1:-1:1,
                ...
                parfor cl = 1:1:2^(l-1),
                    % get the p values for the parent cluster
                    tmpclind = getClind(obj,l,cl);
                    [ppA_temp, ppAPerms_temp, ppAR_temp, ppARPerms_temp] = PPMMV.AngleTest_Permutated([getCOV(obj,COV,tmpclind) obj.XREG],getDepVar(obj,DepVar,tmpclind),AngleTable,t,maxM,0,t2,forcedPerms);
                    clind(cl) = tmpclind;
                    % get the p values for the two children clusters
                    children = getChildren(obj,l,cl);
                    clind1 = getClind(obj,l+1,children(1));
                    ...
                    % nonparametric combination of the p values of the parent and the two children
                    [tmpppA(cl), tmpppAPerms(:,cl)] = PPMMV.nonParametricCombination ([ppA_temp,ppA(clind1),ppA(clind2)], [ppAPerms_temp,ppAPerms(:,clind1),ppAPerms(:,clind2)]); 
                    [tmpppAR(cl), tmpppARPerms(:,cl)] = PPMMV.nonParametricCombination ([ppAR_temp,ppAR(clind1),ppAR(clind2)], [ppARPerms_temp,ppARPerms(:,clind1),ppARPerms(:,clind2)]); 
                end
                ppA(1,clind) = tmpppA;
                ppAPerms(:,clind) = tmpppAPerms;
                ppAR(1,clind) = tmpppAR;
       ...
       function out = runPropagatingAnglePermutatedTest(obj,DepVar,COV,t,t2)   % created by Dorothy 2
            ...
            parfor cl = 1:1:2^(l-1),
                tmpclind = getClind(obj,l,cl);
                [tmpppAP(1,cl), tmpppAPPerms(:,cl)] = PPMMV.AnglePermutatedTest_Permutated([getCOV(obj,COV,tmpclind) obj.XREG],getDepVar(obj,DepVar,tmpclind),t,t2,forcedPerms);
                clind(cl) = tmpclind;
            end
            ...
            for l = obj.nLev-1:-1:1,
                ...
                parfor cl = 1:1:2^(l-1),
                    % get the p values for the parent cluster
                    tmpclind = getClind(obj,l,cl);
                    [ppAP_temp, ppAPPerms_temp] = PPMMV.AnglePermutatedTest_Permutated([getCOV(obj,COV,tmpclind) obj.XREG],getDepVar(obj,DepVar,tmpclind),t,t2,forcedPerms);
                    clind(cl) = tmpclind;
                    % get the p values for the two children clusters
                    children = getChildren(obj,l,cl);
                    clind1 = getClind(obj,l+1,children(1));
                    ...
                    % nonparametric combination of the p values of the parent and the two children
                    [tmpppAP(1,cl), tmpppAPPerms(:,cl)] = PPMMV.nonParametricCombination ([ppAP_temp,ppAP(clind1),ppAP(clind2)], [ppAPPerms_temp,ppAPPerms(:,clind1),ppAPPerms(:,clind2)]); 
                end
                ppAP(1,clind) = tmpppAP;
                ppAPPerms(:,clind) = tmpppAPPerms;
            end
       ...
       function out = featureSelection(obj,X,XC,FS)
           ...
           switch obj.FSelection
               ...
               case 'MIXED'
                   ...
                   order = zeros(3,nFS);
                   info = zeros(3,nFS);
                   [order(1,:),info(1,:)] = PPMMV.fsCOR(FS,XC,nFS);
                   [order(2,:),info(2,:)] = PPMMV.fsROC(FS,X,nFS);
                   [~,~,H] = PPMMV.prepMIfs(FS,X,obj.Quantiles);
                   [order(3,:),info(3,:)] = PPMMV.fsSPECCMI(H,nFS);
                   r = 1:nFS;
                   ranks = repmat(r,3,1);
                   new = info;
                   for i=1:1:3
...
% STATIC METHODS    
    methods (Static = true) % ASSOCIATION ANALYSIS
       ...
       function [pR2, pR2Perms] = testPartialPLSR_Permutated(X,Y,C,t,forcedPerms)        % Created by Dorothy
            ...
            pR2 = sum([R2Perm;R2True] >= repmat(R2True,t+1,1),1)/(t+1);
            pR2Perms = sum(repmat([R2Perm;R2True]',t,1) >= repmat(R2Perm,1,t+1),2)./(t+1);
       end
       function [pA, pAPerm, pAR, pARPerm] = AngleTest_Permutated(IndVar,DepVar,AngleTable,t,maxM,Aval,t2,forcedPerms)    % Created by Dorothy (long computation time)
            ...
            [~,~,STAT] = PPMMV.AngleTest(IndVar,DepVar,AngleTable,t,maxM,Aval);
            pA = STAT.pA;
            pAR = STAT.pAR;
            ...
            for p = 1:1:t2; 
                ...
                [~,~,STAT] = PPMMV.AngleTest(IndVar(perm,:),DepVar,AngleTable,t,maxM,Aval);
                pAPerm(p) = STAT.pA;
                pARPerm(p) = STAT.pAR;
            end
       end
       function [pAP, pAPPerm] = AnglePermutatedTest_Permutated(IndVar,DepVar,t,t2,forcedPerms)     % Created by Dorothy 2
           ...
           for p = 1:1:t2,
               ...
               for k=1:1:2,
                   ind = find(F==k);
                   forM(:,k) = PPMMV.getRegression(IndVarPerm(ind,:),DepVar(ind,:));
               end

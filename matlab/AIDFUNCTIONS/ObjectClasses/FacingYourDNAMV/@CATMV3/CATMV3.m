classdef CATMV3 < PPMMV3
    ...
    methods % CONSTRUCTOR
        function obj = CATMV3(varargin)
            obj = obj@PPMMV3(varargin{:});         
        end
    end
    ...
    methods % CLASSIFIER ANALYSIS
        function trainClassifier(obj,X,FS,varargin)
            ...
            % PREPARATION
              ...
              %if ~any(W)==1, warning('NO WEIGHTS'); return, end % added by Dzemila 18112016
              obj.nTraining = length(ind);
            % SVM LEARNING
              if (~isobject(K)&&~(K==0)), rng(1);K = cvpartition(length(ind),'KFold',K);end
              out = CATMV3.trainSVMRBF(FS(ind,:),X(ind),COST,W(ind),K,obj.Optimize,obj.Standardize,obj.PosClass);
        ...
        function out = testClassifier(obj,X,FS,varargin)
            ...
            % EVALUATION
              [out.EER,out.G,out.AUC,out.XX,out.YY,out.TH,out.Y,out.pAUC] = CATMV3.getEER(PXCN(Tind),PXCN(Find));
              res = PX==Tval;
              [out.HPREC,out.HREC,out.HG,out.HTNF,out.HX,out.HY,out.HACC] = CATMV3.getHardClass(res(Tind),res(Find));
            % VISUALIZATION
              if display
        ...
        function [PX,PXC,PXCN] = predict(obj,FS,nS)
            ...
            % PREDICT  
              ...
              %PXCN = PXC;
              PXCN = CATMV3.statNormPXC(PXC,obj.OffsetPXC,obj.ScalePXC);
        end
    ...
    methods (Static = true)
        function out = trainSVMRBF(FS,X,COST,W,K,opt,Stand,Tval)
            ...
            if Stand
               ...
               [FS,out.AvgFS,out.StdFS] = PPMMV3.standardize(FS);
            else
            ...
            tmp = fitcsvm(FS,X,'Standardize',false,'KernelFunction','rbf','BoxConstraint',C,...
                          'KernelScale','auto','Cost',COST,'Weights',W,'Prior',prior);
            sigma = tmp.KernelParameters.Scale;          
            if ~isobject(K)&&~(K==0), K = cvpartition(length(X),'KFold',K); end          
            switch opt
                case false
                    out.Classifier = fitcsvm(FS,X,'Standardize',false,'KernelFunction','rbf','BoxConstraint',C,...
                                             'KernelScale',sigma,'Cost',COST,'Weights',W,'Prior',prior);                                       
                case true
                    init.scale = log(sigma);
                    init.c = log(1);
                    [out.cv,sigma,C] = CATMV3.optSVMRBF(FS,X,COST,W,K,init);
                    out.Classifier = fitcsvm(FS,X,'Standardize',false,'KernelFunction','rbf','BoxConstraint',C,...
                                             'KernelScale',sigma,'Cost',COST,'Weights',W,'Prior',prior);                     
            end
...
%                                'KernelScale',sigma));
            % CROSS VALIDATION EER
             if isobject(K)
                [out.AUC,out.pAUC] = CATMV3.kfoldEER(FS,X,K,COST,W,C,sigma,Tval,false);
             else
                 out.AUC = [];
                 out.pAUC = [];
             end
        ...
        function [auc,pauc] = kfoldEER(fs,x,K,COST,w,C,sigma,Tval,display)
                 ...
                 for i=1:K.NumTestSets
                     ...
                     [~,~,auc(i),X,Y,~,~,pauc(i)] = CATMV3.getEER(PXCN(Tind),PXCN(Find));% CHECK ORDER
                     if display

function out = METAextractGWASPeaks(GWAS,dist,LD,pCrit,DB,display)
    ...
    while ~isempty(list)&&lab<=length(index)
          ...
          for r=1:length(subind)
              ...
              NID(r) = length(indr);
              %lm = fitlm(bestsnp(indr),inj(indr),'linear');
              %CORR(r) = lm.Rsquared.Ordinary;

...
%close all;clear all;
function GWAS = LoadMVPerSyndromeGWASmichp03(nSyndromes)  
    %nSyndromes = 61;
    studypath = '/IMAGEN/FACE/EUROGWAS/';
    disp('LOADING MICHP03 VERSION');
    INPUTPATH = [studypath 'GENOTYPES/SNPJOBSINTERSECTpoint8/'];
    OUTPUTPATH = [studypath 'GENOTYPES/OUTPUTINTERSECT_MVSYN_PerSyndrome/'];
    ...
    DB = cell(1,2);
    nDB = 2;
    DB{1}.nSegments = nSyndromes;
    DB{2}.nSegments = nSyndromes;
    ...
    P = zeros(nSNPperJOB,nSyndromes,nDB,nDB+1,nJOBS,'uint32');
...
%            end
        ...
        in = load(outputfile);
        in = in.in;
        %p = cat(3,in.pvalues,in.metapvaluesIV);
        p = in.pvalues(:,1:nSyndromes,:,:);p(:,:,:,3) = in.pfastvalues(:,1:nSyndromes,:);
...
%     SIGN = SIGN(index,:,:);
    ...
    clear CHR POS RS A1 A2 P N MAF;
end
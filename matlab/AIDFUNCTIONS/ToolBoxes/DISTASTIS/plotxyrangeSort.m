function plotxyrange(F,axe1,axe2,titre,noms,range,lc);
...
%axis;
for i=1:nrow/2,
plot ( F(i,axe1),F(i,axe2 ),'.');
end
for i=1+nrow/2:nrow,
plot ( F(i,axe1),F(i,axe2 ),'r.');
end
...
labelx=[label, num2str(axe1), ' (', num2str(round(1000*lc(1)/sum(lc))/10),'%)'];
labely=[label, num2str(axe2)  ' (', num2str(round(1000*lc(2)/sum(lc))/10),'%)'];
xlabel (labelx);
ylabel (labely );
...
for i=1:nrow/2,
  text(F(i,axe1),F(i,axe2),noms{i,1},'color','b');
end;
for i=1+nrow/2:nrow
  text(F(i,axe1),F(i,axe2),noms{i,1},'color','r');
end;
%title(titre);

function [out,totalerror]= rigidMatchW(a,b,w)
...
if (nrpoints>0)
    ...
    for i=1:nrpoints
        centera=centera+w(i)*a(:,i);
        centerb=centerb+w(i)*b(:,i);
    end
    centera=centera/totalweight;
    centerb=centerb/totalweight;
    ...
    %iterate rotation
    totalerror=calcep_w(a,b,w);

    while (theta>mintheta)
        M=calcmoment_w(a,b,w);
        if (M==0)
            break;
        end
        M=M/norm(M);

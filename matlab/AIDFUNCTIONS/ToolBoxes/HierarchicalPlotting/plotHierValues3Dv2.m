function [ v ] = plotHierValues3Dv2(values,varargin)
...
%------------- SCREENING INPUT ----------------%
    ...
    for L = 1:(log2(length(values)+1)-1)
        ...
        if DRAW_CONCENTRICS
            ...
            plot3(plotax,xp,yp,zp,'Color',linecolor,'LineStyle','-','LineWidth',LINE_WIDTH);
            hold(plotax,'on');
        end
        ...
        for P = 1:2^(L-1)
            plot3(plotax,[previous_points(1,P),point_list(1,(P-1)*2+1)],....
                               ...
                               [previous_points(3,P),point_list(3,(P-1)*2+1)],'Color',linecolor,'linewidth',LINE_WIDTH,'LineStyle','-');
            plot3(plotax,[previous_points(1,P),point_list(1,(P-1)*2+2)],...
                               [previous_points(2,P),point_list(2,(P-1)*2+2)],...
                               [previous_points(3,P),point_list(3,(P-1)*2+2)],'Color',linecolor,'linewidth',LINE_WIDTH,'LineStyle','-');
        end 
    ...
    for n=1:size(circle_centers,2)
        ...
        if ~isempty(crit)
            if values(n) >= crit
                scan.Material = 'Dull';
                plot3(plotax,xp,yp,zp,'Color',MARKER_COLOR_POSITIVE,'LineStyle','-','LineWidth',LINE_WIDTH*3);
            else
                scan.Material = 'Dull';
        ...
        pointx = maxx(1)-(4/5)*(maxx(1)-minx(1));
        %pointx = circle_centers(1,n);
        %pointx = minx;
        pointy = circle_centers(2,n);
        ...
        text(pointx,pointy,pointz,labels{n},'Color','k','FontSize',13,'Parent',plotax);
    end
    view(plotax,0,90);
    %plotax.Visible = 'off';
    plotax.Color = [1 1 1];

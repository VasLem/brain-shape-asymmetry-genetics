function [ v ] = plotHierValues3Dv4(values,varargin)
...
%------------- SCREENING INPUT ----------------%
    ...
    for L = 1:(log2(length(values)+1)-1)
        ...
        if DRAW_CONCENTRICS
            ...
            %plot3(plotax,xp,yp,zp,'Color','k','LineStyle',':','LineWidth',LINE_WIDTH);
            plot3(plotax,xp,yp,zp,'Color',[0.6 0.6 0.6],'LineStyle',':','LineWidth',LINE_WIDTH);
            hold(plotax,'on');
        end
...
%                                [previous_points(3,P),point_list(3,(P-1)*2+2)],'k','linewidth',LINE_WIDTH);
            plot3(plotax,[previous_points(1,P),point_list(1,(P-1)*2+1)],....
                               [previous_points(2,P),point_list(2,(P-1)*2+1)],...
                               [previous_points(3,P),point_list(3,(P-1)*2+1)],'Color',[0.6 0.6 0.6],'linewidth',LINE_WIDTH);
            plot3(plotax,[previous_points(1,P),point_list(1,(P-1)*2+2)],...
                               [previous_points(2,P),point_list(2,(P-1)*2+2)],...
                               [previous_points(3,P),point_list(3,(P-1)*2+2)],'Color',[0.6 0.6 0.6],'linewidth',LINE_WIDTH);
        end 
        ...
        if ~isempty(crit)
            ...
            for c=nCrit:-1:1
                if values(n) >= crit(c)
                    ...
                    plot3(plotax,xp,yp,zp,'Color',[0 0 0],'LineStyle','-','LineWidth',LINE_WIDTH*3);
                    break;
                end
            end            
        end

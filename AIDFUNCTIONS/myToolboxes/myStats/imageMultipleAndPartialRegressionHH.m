function imageMultipleAndPartialRegressionHH(Result,RefScan,vref,name)
%% Multiple effect
try
...
for T=1:1:length(Result.PT)
   if isempty(Result.PT(T)), continue; end
    ...
    try
    v = viewer(RefScan);v.BackgroundColor = [1 1 1];v.SceneLightLinked = true;v.SceneLightVisible = true;syncCamera(vref,v);
    RefScan.Value = double((Result.PT(T).pLocalR2<=0.05))';colormap(v.RenderAxes,'summer');set(v.Figure,'Position',get(vref.Figure,'Position'));

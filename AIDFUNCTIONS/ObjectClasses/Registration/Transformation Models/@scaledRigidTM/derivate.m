...
%               p = p - repmat(obj.c,1,nr);
              grad = obj.Scale*dTdRx(obj)*p;
          case 2
%               if isempty(obj.c),obj.c = p;end
%               p = p - repmat(obj.c,1,nr);
              grad = obj.Scale*dTdRy(obj)*p;
          case 3
%               if isempty(obj.c),obj.c = p;end
%               p = p - repmat(obj.c,1,nr);
              grad = obj.Scale*dTdRz(obj)*p;
          case 4
              grad = obj.Scale*repmat(dTdTx(obj),1,nr);
          case 5
              grad = obj.Scale*repmat(dTdTy(obj),1,nr);
          case 6
              grad = obj.Scale*repmat(dTdTz(obj),1,nr);
          case 7
              grad = (obj.Rotation*(p)+repmat(obj.Translation,1,nr));
          otherwise
              if nargout == 1, out = [];end
              return;
      end

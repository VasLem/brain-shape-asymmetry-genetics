classdef SNPMV2 < CATMV2
   ...
   methods % GETTING/SETTING
       ...
       function out = get.GGGTBal(obj)
          if isempty(obj.GT), out = []; return; end
          out = SNPMV2.getGGGTBalance(obj.GG,obj.GT);
       end
       ...
       function out = get.Balances(obj)
           ...
           out(1) = SNPMV2.getBalance(obj.nAA+obj.nAB,obj.nBB);% Recessive
           out(2) = SNPMV2.getBalance(obj.nBB+obj.nAB,obj.nAA);% Dominant
           out(3) = SNPMV2.getBalance(obj.nAA+obj.nBB,obj.nAB);% Over Dominant
           out(4) = SNPMV2.getBalance(obj.nAA,obj.nBB);% AA versus BB
           out(5) = SNPMV2.getBalance(obj.nAA,obj.nAB);% AA versus AB
           out(6) = SNPMV2.getBalance(obj.nAB,obj.nBB);% AB versus BB
       end
   ...
   methods % INTERFACING
       function out = getVarInfo(obj,DepVar)
           ...
           out.PosClass = obj.PosClass;% needed for ROC analysis (minority group definition)
       end
       ...
       function [X,COST,W,ind,Tind,Find] = prepPredictorTraining(obj,X) %#ok<*INUSL>
            X = GT2GG(obj,X);
            %ind = find(~isnan(X));
            %ind = find(abs(X));% When testid = 4, heterozygotes are excluded
       ...
       function postFeatureLearning(obj)
           [obj.MODBal,obj.MODMinBal] = SNPMV2.getGGBalance(obj.XMOD); % GG without nan is coded in XMOD
       end
       function out = getFreq(obj,in) %#ok<*INUSD>
           if in==0, out = nan; return; end
           Tind = obj.GG==in;
           Find = obj.GG==-1*in;
           out = sum(Tind)/(sum(Tind)+sum(Find));
           %out = sum(Tind)/length(obj.GG);
       end
       function out = prepBiometricTest(obj,X)
           out = GT2GG(obj,X);
           out(out==0) = nan;% remove heterozygotes
       end

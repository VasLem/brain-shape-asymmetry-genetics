classdef BIOMETRICSMV3 < superClassLight
    properties
       PPM = {};
    end
    ...
    methods % CONSTRUCTOR
        function obj = BIOMETRICSMV3(varargin)
    ...
    methods % GENERAL SETTING/GETTING
        ...
        function out = get.SEXInd(obj)
            out = getBININD(obj,'sex');
        end
        function out = get.BINInd(obj)
            tmp = BINMV3;
        ...
        function out = get.nBIN(obj)
            out = length(obj.BINInd); 
        end
        function out = get.AGEInd(obj)
            out = getBININD(obj,'age');
        end
        ...
        function out = get.BMIInd(obj)
            out = getBININD(obj,'BMI');
        end
        function out = get.nBMI(obj)
            out = length(obj.BMIInd); 
        end
    ...
    methods % INTERFACING
        ...
        function out = getBININD(obj,what)
            tmp = getVarNames(obj,obj.BINInd);
            ind = zeros(1,length(tmp));
            for i=1:1:length(tmp)
               if ~isempty(strfind(tmp{i},what)),ind(i) = 1;end 
            end
            if sum(ind)==0, out = []; return; end
            out = obj.BINInd(find(ind)); %#ok<*FNDSB>
        end
    end
    methods % BIOMETRICS
        function biometricMatchesWeights(obj,TestDepVar,TestCOV,TestGB,TestGT,TestRS)
               % INITIALIZING
                % TestGT = TestSNP;
                 L1CL1TestDepVar = BIOMETRICSMV3.getL1CL1DepVar(TestDepVar);
                 ...
                 switch obj.GroupNorm
                     case true
                         ...
                         for i=1:1:nrT
                             % SEX
                             if ~isempty(sexind)
                                wf(sexind,i) = BIOMETRICSMV3.sum2one(wf(sexind,i)); 
                                wfh(sexind,i) = BIOMETRICSMV3.sum2one(wfh(sexind,i));
                                wh(sexind,i) = BIOMETRICSMV3.sum2one(wh(sexind,i));
                             end
                             % AGE
                             if ~isempty(ageind)
                                 wf(ageind,i) = BIOMETRICSMV3.sum2one(wf(ageind,i)); 
                                 wfh(ageind,i) = BIOMETRICSMV3.sum2one(wfh(ageind,i));
                                 wh(ageind,i) = BIOMETRICSMV3.sum2one(wh(ageind,i));
                             end
                             % HEIGHT 
                             if ~isempty(hind)
                                 wf(hind,i) = BIOMETRICSMV3.sum2one(wf(hind,i)); 
                                 wfh(hind,i) = BIOMETRICSMV3.sum2one(wfh(hind,i));
                                 wh(ageind,i) = BIOMETRICSMV3.sum2one(wh(ageind,i));
                             end
                             % WEIGHT 
                             if ~isempty(wind)
                                 wf(wind,i) = BIOMETRICSMV3.sum2one(wf(wind,i)); 
                                 wfh(wind,i) = BIOMETRICSMV3.sum2one(wfh(wind,i));
                                 wh(wind,i) = BIOMETRICSMV3.sum2one(wh(wind,i));
                             end
                             % BMI 
                             if ~isempty(bmiind)
                                 wf(bmiind,i) = BIOMETRICSMV3.sum2one(wf(bmiind,i)); 
                                 wfh(bmiind,i) = BIOMETRICSMV3.sum2one(wfh(bmiind,i));
                                 wh(bmiind,i) = BIOMETRICSMV3.sum2one(wh(bmiind,i));
                             end
                             % GBBACKGROUND
                             if ~isempty(gbind)
                                 wf(gbind,i) = BIOMETRICSMV3.sum2one(wf(gbind,i)); 
                                 wfh(gbind,i) = BIOMETRICSMV3.sum2one(wfh(gbind,i));
                                 wh(gbind,i) = BIOMETRICSMV3.sum2one(wh(gbind,i));
                             end
                             % SNPs
                             if ~isempty(snpind)
                                 if isempty(ggind)
                                    wf(snpind,i) = BIOMETRICSMV3.sum2one(wf(snpind,i)); 
                                    wfh(snpind,i) = BIOMETRICSMV3.sum2one(wfh(snpind,i));
                                    wh(snpind,i) = BIOMETRICSMV3.sum2one(wh(snpind,i));
                                 else
                                    gg = unique(ggind); 
                                    ngg = length(gg);
                                    for j=1:1:ngg
                                        ...
                                        wf(snpind(ind),i) = avwf*BIOMETRICSMV3.sum2one(wf(snpind(ind),i)); 
                                        wfh(snpind(ind),i) = avwfh*BIOMETRICSMV3.sum2one(wfh(snpind(ind),i));
                                        wh(snpind(ind),i) = BIOMETRICSMV3.sum2one(wh(snpind(ind),i));
                                    end
                                    %wf(snpind,i) = BIOMETRICSMV3.sum2one(wf(snpind,i)); 
                                    %wfh(snpind,i) = BIOMETRICSMV3.sum2one(wfh(snpind,i));
                                    %wh(snpind,i) = BIOMETRICSMV3.sum2one(wh(snpind,i));
...
%                         tmpwf(:,c) = BIOMETRICSMV3.sum2one(tmpwf(:,c)); 
%                      end
%                      for c=1:1:size(tmpwf,1)
%                         tmpwf(c,:) = BIOMETRICSMV3.sum2one(tmpwf(c,:)); 
%                      end

                     [Ranks(:,r,:),EER(:,r),~,~,X(r,:),Y(r,:),AUC(:,r),~] = BIOMETRICSMV3.getBiometrics(TScores(ind,:),FScores(ind,:,:),tmpwf);
                     r=r+1;
                     tmpwfh = wfh(ind,:);
%                      for c=1:1:size(tmpwf,2)
%                         tmpwfh(:,c) = BIOMETRICSMV3.sum2one(tmpwfh(:,c)); 
%                      end
%                      for c=1:1:size(tmpwf,1)
%                         tmpwfh(c,:) = BIOMETRICSMV3.sum2one(tmpwfh(c,:)); 
%                      end
                     [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BIOMETRICSMV3.getBiometrics(TScores(ind,:),FScores(ind,:,:),tmpwfh);
                     [FM{i}.A,FM{i}.evalA,FM{i}.D,FM{i}.evalD,FM{i}.X] = BIOMETRICSMV3.evalSortedResults(ST,SF,L1CL1TestDepVar,T);
                 end
                 ...
                 if vis
                    str = {'g-.' 'g-' 'b-.' 'b-' 'r-.' 'r-' 'k-.' 'k-' 'm-.' 'm-' 'c-.' 'c-' 'y-.' 'y-'}; 
        ...
        function out = biometricPerformancev2(obj,TestDepVar,uindex,vis,index)
                 ...
                 for i=1:1:nG 
                     ...
                     tmpwfh = wfh(ind,:);
                     [Ranks(:,r,:),EER(:,r),ST,SF,X(r,:),Y(r,:),AUC(:,r),T] = BIOMETRICSMV3.getBiometrics(TScores(ind,:),FScores(ind,:,:),tmpwfh);
                     [FM{i}.A,FM{i}.evalA,FM{i}.D,FM{i}.evalD,FM{i}.X] = BIOMETRICSMV3.evalSortedResultsv2(ST,SF,L1CL1TestDepVar,T,uindex);
                 end
                 Ranks(1,:,:) = (Ranks(1,:,:)./nrT).*100;
                 CumRanks = zeros(1,nr,100);
                 for cr=1:1:100
    ...
    methods % EVOMORPH
        function out = matchSolutions(obj,X,DepVar,index)
                 L1CL1DepVar = BIOMETRICSMV3.getL1CL1DepVar(DepVar);
    ...
    methods (Static = true)
        ...
        function [R,EER,ST,SF,X,Y,AUC,T] = getBiometrics(TScores,FScores,wM)
                 ...
                 ST = BIOMETRICSMV3.accScores(TScores,wM)';
                 SF = BIOMETRICSMV3.accScores(FScores,repmat(wM,1,1,nF));
                 [R,EER,X,Y,AUC,T] = BIOMETRICSMV3.evalScores(ST,SF,nT,nF); 
        end

classdef JanusEvoMorphEDAGK < superClass
    ...
    properties %re-production
        Reduction = 500; % Reduced population size
        ScalingFcn = 'Rank';
    ...
    methods % Constructor
        function obj = JanusEvoMorphEDAGK(varargin)
    ...
    methods % Special Setting & Getting
        ...
        function out = get.pFaces(obj)
            out = sum(((obj.Population-repmat(obj.FaceModel.AvgCoeff',obj.PopulationSize,1))./repmat(obj.FaceModel.EigStd',obj.PopulationSize,1)).^2,2)'/obj.FaceModel.nrEV;
        end
    ...
    methods % General Interface Functions for the evolution proccess
        ...
        function InitializePopulation(obj)
            if isempty(obj.FaceModel), 
                error('First Load Face Model'); 
            end
            for i=1:obj.PopulationSize
                obj.Population(i,:) = normrnd(obj.FaceModel.AvgCoeff',obj.FaceModel.EigStd');
            end            
        end
        function evalPopulation(obj)
            obj.Scores = (1-obj.FacePenalty)*getScore(obj.DemiModelSet,obj.Population') + obj.FacePenalty*obj.pFaces;
        end
        ...
        function nextGeneration(obj)
            obj.Population = obj.Population(obj.sortIndex,:);
            PopR = obj.Population(1:obj.Reduction,:);
            RemainSize = obj.PopulationSize - obj.Reduction;
            for i=1:obj.FaceModel.nrEV
                [~,~,h] = ksdensity(PopR(:,i));
                mark = randsample(obj.Reduction,RemainSize,true);
                obj.Population(obj.Reduction+1:obj.PopulationSize,i) = PopR(mark,i)+h.*randn([RemainSize 1]);                
            end
        end
        ...
        function stopTest(obj)
            ...
            %if obj.OldBestScore>obj.BestScore,
            %    obj.OldBestScore = obj.BestScore;
            %    obj.OldPopulation = obj.Population;
            %    obj.OldScores = obj.Scores;
            %    obj.OldGeneration = obj.Generation;
            %    obj.nrNoImprove = 0;
            %else
            %    obj.nrNoImprove = obj.nrNoImprove+1;
            %end
            %if obj.nrNoImprove>=obj.Tol,
            %    obj.State = 'Stop';
            %    obj.Population = obj.OldPopulation;
            %    obj.Scores = obj.OldScores;
            %    obj.Generation = obj.OldGeneration;
            %end
        end

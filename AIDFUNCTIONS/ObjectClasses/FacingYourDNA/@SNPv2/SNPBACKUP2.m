classdef SNP < superClass
   ...
   % STAGE 3: EXTRACTING PREDICTION MODEL
   properties
      ST3TestInd = 0;
      ST3F = 0;
      ST3P = 1;
       ...
       function obj = runStage1(obj,t,DepVar,parcont)
           ...
           for i=1:1:7
               try
                  ...
                  obj.ST1F(i) = stat;obj.ST1P(i) = p;
               catch
                  obj.ST1F(i) = nan;obj.ST1P(i) = nan;
               end
           end
       end
       function obj = runStage2(obj,DepVar,Cov,parcont)
           if nargin==4, obj.ParCont = parcont;end
           ...
           for i=1:1:7
               try
                 out = SNPBRIM(obj,i,DepVar,Cov,Fouter);
                 obj.ST2P(i) = out.FP;obj.ST2F(i) = out.F;
                 FFold(:,i) = out.FoldF(:);FPFold(:,i) = out.FoldFP(:);
               catch %#ok<*CTCH>
                 obj.ST2P(i) = nan;obj.ST2F(i) = nan;
                 FFold(:,i) = nan*zeros(obj.OuterFold,1);FPFold(:,i) = nan*zeros(obj.OuterFold,1);
               end
           end
           obj.ST2FFold = FFold;obj.ST2PFold = FPFold;
       end
       function obj = runStage3(obj,BaseCont,UseRedShape,parcont)
           try
               if nargin==4, obj.ParCont = parcont; end
               ...
               % Remember shape space used  
                 obj.ST3RedShape = UseRedShape;
               % Run SNPBRIM 
                 obj.ST3MOD = SNPBRIM(obj,obj.ST3TestInd,DepVar,Cov);
                 obj.ST3MOD = rmfield(obj.ST3MOD,{'nrAdded' 'DepVarAdd' 'IndVarAdd' 'GMAdd' 'BootIter' 'FoldF' 'FoldFP' 'DepVar' 'IndVar','Grp','H'});
               % Setting up reference scan 
                 RIPref = SNP.updateRIP(RefCoeff,obj.ST3MOD.M);
                 RIPref = SNP.normalizeRIP(RIPref,obj.ST3MOD.M,obj.ST3MOD.Var);
                 obj.ST3MOD.RIPRef = RIPref;
               % Complete population
                 %RIP = SNP.updateRIP(DepVar,obj.ST3MOD.M);
                 %RIP = SNP.normalizeRIP(RIP,obj.ST3MOD.M,obj.ST3MOD.Var);
                 %obj.ST3MOD.CompRIP = RIP;  
               % Population used to model things
                 RIP = SNP.updateRIP(DepVar(obj.ST3MOD.UsedInd,:),obj.ST3MOD.M);
                 RIP = SNP.normalizeRIP(RIP,obj.ST3MOD.M,obj.ST3MOD.Var);
                 obj.ST3MOD.RIP = RIP;
                 [obj.ST3MOD.RIPF] = SNP.performRIPStat(obj.ST3MOD.GG,RIP,obj.ST3MOD.Var);
               % Obtaining regressions to contruct morphs
                 IndVar = [Cov(obj.ST3MOD.UsedInd,:) RIP'];
                 [obj.ST3MOD.MMorphs,obj.ST3MOD.MIMorphs,~] = getRegression(IndVar,DepVar(obj.ST3MOD.UsedInd,:),(1:size(IndVar,2)));
                 %%[obj.ST3MOD.MMorphsOrig,obj.ST3MOD.MIMorphsOrig,obj.ST3MOD.R2MorphsOrig] = getRegression(IndVar,OrigDepVar(obj.ST3MOD.UsedInd,:),(1:size(IndVar,2)));
                 %popShape = repmat(ShapeSpace.AvgVec,1,length(obj.ST3MOD.UsedInd)) + ShapeSpace.EigVec*DepVar(obj.ST3MOD.UsedInd,:)';
                 ...
                 %end
                 %obj.ST3MOD.R2 = R2;
                 %obj.ST3MOD.R2LM = sum(obj.ST3MOD.R2);
               % Obtaining group memberships and additional needed info
                 ...
                 switch obj.ST3TestInd
                     case 1 % RECESSIVE
                         GTInfo{1}.Ind = find(obj.ST3MOD.GG==-1);
                         GTInfo{1}.Sign = -1;
                         GTInfo{1}.Opp = 3;
                         GTInfo{2}.Ind = find(obj.ST3MOD.GG==-1);
                         GTInfo{2}.Sign = -1;
                         GTInfo{2}.Opp = 3;
                         GTInfo{3}.Ind = find(obj.ST3MOD.GG==1);
                         GTInfo{3}.Sign = 1;
                         GTInfo{3}.Opp = 1;
                     case 2 % DOMINANT
                         GTInfo{1}.Ind = find(obj.ST3MOD.GG==-1);
                     ...
                     case 4 % ADDITIVE
                         ...
                         GTInfo{1}.Sign = -1;
                         GTInfo{1}.Opp = [2 3];
                         GTInfo{2}.Ind = find(obj.ST3MOD.GG==0);
                         GTInfo{2}.Sign = 0;
                         GTInfo{2}.Opp = [1 3];
                         GTInfo{3}.Ind = find(obj.ST3MOD.GG==1);
                         GTInfo{3}.Sign = 1;
                 ...
                 GTInfo{1}.Freq = length(GTInfo{1}.Ind)/length(find(~isnan(obj.ST3MOD.GG)));
                 GTInfo{2}.Freq = length(GTInfo{2}.Ind)/length(find(~isnan(obj.ST3MOD.GG)));
                 GTInfo{3}.Freq = length(GTInfo{3}.Ind)/length(find(~isnan(obj.ST3MOD.GG)));
               % Extracting group RIP Statistics  
                 for i=1:1:3
                     GTInfo{i}.Nr = length(GTInfo{i}.Ind);
                     if GTInfo{i}.Nr>0
                        GTInfo{i}.RIP = obj.ST3MOD.RIP(GTInfo{i}.Ind);
                        [GTInfo{i}.AVG,GTInfo{i}.STD,GTInfo{i}.MED,GTInfo{i}.MAD,GTInfo{i}.O] = SNP.extractRobustRIPStatistics(GTInfo{i}.RIP,6);                     
                     end
                 end
                 obj.ST3MOD.GTInfo = GTInfo;
                 obj.ST3MOD.Freq = [GTInfo{1}.Freq GTInfo{2}.Freq GTInfo{3}.Freq];
               % Creating average morphs
                 %obj.ST3MOD.Atyps = nan*zeros(1,3);
                 %for i=1:1:3
                 %    if obj.ST3MOD.GTInfo{i}.Nr==0, continue; end
                 %    [obj.ST3MOD.GTInfo{i}.C,obj.ST3MOD.GTInfo{i}.Atyp,obj.ST3MOD.GTInfo{i}.Morph] = createMorph(obj,BaseCont,obj.ST3MOD.GTInfo{i}.GT,0);
                 %    obj.ST3MOD.Atyps(i) = obj.ST3MOD.GTInfo{i}.Atyp;
                 %end
               % Determination of less typical group
                 %obj.ST3MOD.MAtyp = nanmax(obj.ST3MOD.Atyps);
                 %obj.ST3MOD.mAtyp = nanmin(obj.ST3MOD.Atyps);
                 %obj.ST3MOD.BalAtyp = obj.ST3MOD.mAtyp/obj.ST3MOD.MAtyp;   
           catch
               obj.ST3MOD = [];
           end
       end
   ...
   methods % Interfacing Functions
               ...
               case 2
                   switch stat
                       case 'F' % Using F statistics 
                            code = obj.ST2FCode;
                       case 'G' % Using G statistics % REMOVED
                            code = obj.ST2GCode;
                   end
                   MH = 1;
               case 3
       ...
       function out = getShapeEffects(obj,BaseCont)
           ...
           obj.ST3MOD.R2 = R2;
           obj.ST3MOD.R2LM = sum(obj.ST3MOD.R2);
           obj.ST4Ref.Scan.Value = obj.ST3MOD.R2LM;
           out = [];
       end
       function obj = reduceSamples(obj,index)
           ...
           obj.GT = obj.GT(index);
           try
            obj.ST3MOD.CompRIP = obj.ST3MOD.CompRIP(index);
            [ism,loc] = ismember(obj.ST3MOD.UsedInd,index);
            obj.ST3MOD.RIP = obj.ST3MOD.RIP(find(ism));
            obj.ST3MOD.UsedInd = loc(find(ism));
            obj.ST3MOD.GM = obj.ST3MOD.GM(find(ism));
           catch
           end
       end
   end
            ...
            % Outer Partitioning of Data
              ...
              parfor fo=1:ParInfo.OuterFold
                 ...
                 % Evaluate outer test data
                  ...
                  [FoldResults{fo}.F,FoldResults{fo}.FP] = SNP.performRIPStat(FoldResults{fo}.IndVar,FoldResults{fo}.RIP,var);
              end
             % Gathering Fold Results
               ...
               out.FoldF = FoldF;out.FoldFP = FoldFP;out.FoldFP(out.FoldFP==0) = 0.000000000001;
               out.FP = SNP.pfast(out.FoldFP);out.F = nanmean(FoldF);
   ...
   methods % Model functions
       function [C,Atypicality,scan] = createMorph(obj,BaseCont,genotype,BF,maxAtypicality)
                ...
                if isempty(obj.ST3MOD), return; end
                switch genotype
                    case -1 % AA
                        GTInfo = obj.ST3MOD.GTInfo{1};
                    case 0  % AB
                        GTInfo = obj.ST3MOD.GTInfo{2};
                    case 1  % BB
                        GTInfo = obj.ST3MOD.GTInfo{3};
                    otherwise
                        return;
                end
                if GTInfo.Nr==0, return; end
                if obj.ST3RedShape
                   Xref = obj.ST3MOD.RIPRef;
                ...
                while Atypicality>=maxAtypicality && counter<=maxcounter
                      ...
                      deltaX = X-Xref;dY = deltaX*obj.ST3MOD.MMorphs;
                      C = Yref+dY;
       ...
       function [C,Atypicality,scan] = overLaySNP(obj,BaseCont,genotype,Ref,BF,maxAtyp,adjust,TrimBF)
                ...
                if GTInfo.Nr==0, return; end
                Xref = SNP.updateRIP(Ref.Coeff,obj.ST3MOD.M);
                Xref = SNP.normalizeRIP(Xref,obj.ST3MOD.M,obj.ST3MOD.Var);
       ...
       function [IB,pTpF,pT] = matchRIP(obj,rip,genotype,BF)
                 ...
                 for i=1:1:length(GTInfo.Opp) 
                     OppInfo = obj.ST3MOD.GTInfo{GTInfo.Opp(i)};
       ...
       function IB = inversematchRIP(obj,rip,genotype,BF)
                 n = length(rip);IB = nan*zeros(1,n);
       ...
       function [IB,pTIB,pT] = matchFaces(obj,faces,genotype,BF)
                rip = SNP.updateRIP(faces,obj.ST3MOD.M);
                rip = SNP.normalizeRIP(rip,obj.ST3MOD.M,obj.ST3MOD.Var);
       ...
       function [class] = classifyRIP(obj,rip)
                ...
                if obj.ST3MOD.Var.nrEl==3, return; end % cannot classify
                rip = rip*obj.ST3MOD.Var.PosClass;
                class = -1*ones(1,n);
                class(rip > obj.ST3MOD.RIPGT) = 1;
                class = class*obj.ST3MOD.Var.PosClass;
       end
       ...
       function out = illustrateModel(obj,BF)
           ...
           disp(obj.ST4Label);
           disp('   ');
           disp(['Atyp: ' num2str(obj.ST3MOD.Atyps) '  Balance:' num2str(obj.ST3MOD.BalAtyp)]);
           disp('   ');
           disp(['G: ' num2str(obj.ST3MOD.RIPG)  '  GT: '  num2str(obj.ST3MOD.RIPGT)]);
           disp('   ');
           disp(['FP: ' num2str(obj.ST3MOD.FP)  '  GP: '  num2str(obj.ST3MOD.GP)]);
           disp('   ');
           disp(num2str([obj.nrAA obj.nrAB obj.nrBB]));
           disp('   ');
           rip = min(obj.ST3MOD.RIP):0.01:max(obj.ST3MOD.RIP);
           ...
           %data = zeros(3,4);
           for i=1:1:3
               if obj.ST3MOD.GTInfo{i}.Nr == 0, disp(['NO ' num2str(gt(i))]); continue; end
               [score] = matchRIP(obj,rip,gt(i),BF);
               plot(f1.CurrentAxes,rip,score);
               [M(1,i),ind] = min(score);Mrip(1,i) = rip(ind);
               %[score] = matchRIP(obj,obj.ST3MOD.GTInfo{i}.MED.mu,gt(i),BF);
               %data(i,1) = obj.ST3MOD.GTInfo{i}.MED.mu;
               %data(i,2) = obj.ST3MOD.GTInfo{i}.MAD.mu;
               %data(i,3) = obj.ST3MOD.GTInfo{i}.MED.mu+obj.ST3MOD.GTInfo{i}.Sign*obj.ST3MOD.GTInfo{i}.MAD.mu;
               %data(i,4) = score;
               %scan = obj.ST3MOD.GTInfo{i}.Morph;
               %color = 0.6*ones(1,3);color(i) = 0.8;
               %scan.SingleColor = color;scan.Material = 'Dull';
               %v = viewer(obj.ST3MOD.GTInfo{i}.Morph);
           ...
           out.Atyp = obj.ST3MOD.Atyps;
           out.G = obj.ST3MOD.RIPG;
       end
       function [freq,occ] = GTTypicality(obj,genotype)
           ...
           switch genotype
               case -1
                   occ = (obj.nrAA/total);
                   %atyp = obj.ST3MOD.Atyps(1);
                   freq = obj.ST3MOD.Freq(1);
               case 0
                   occ = (obj.nrAB/total);
                   %atyp = obj.ST3MOD.Atyps(2);
                   freq = obj.ST3MOD.Freq(2);
               case 1
                   occ = (obj.nrBB/total);
                   %atyp = obj.ST3MOD.Atyps(3);
                   freq = obj.ST3MOD.Freq(3);
               otherwise
                   occ = nan;
                   %atyp = nan;
                   freq = nan;
       ...
       function [RIP,Distr] = GT2RIP(obj,genotype)
                 ...
                 switch genotype
                     case -1
                         Info = obj.ST3MOD.GTInfo{1};
                     case 0
                         Info = obj.ST3MOD.GTInfo{2};
                     case 1
                         Info = obj.ST3MOD.GTInfo{3};
                     otherwise
                         return;
                 end
       ...
       function updateGTDistributions(obj,BaseCont,kappa)
                % Extracting group RIP Statistics  
                 for i=1:1:3
                     obj.ST3MOD.GTInfo{i}.Nr = length(obj.ST3MOD.GTInfo{i}.Ind);
                     if obj.ST3MOD.GTInfo{i}.Nr>0
                        obj.ST3MOD.GTInfo{i}.RIP = obj.ST3MOD.RIP(obj.ST3MOD.GTInfo{i}.Ind);
                        [obj.ST3MOD.GTInfo{i}.AVG,obj.ST3MOD.GTInfo{i}.STD,obj.ST3MOD.GTInfo{i}.MED,obj.ST3MOD.GTInfo{i}.MAD,GTInfo{i}.O] = SNP.extractRobustRIPStatistics(obj.ST3MOD.GTInfo{i}.RIP,kappa);                     
                     end
                 end
                % Creating average morphs
                 obj.ST3MOD.Atyps = nan*zeros(1,3);
   ...
   methods (Static = true)
       ...
       function [F,FP] = performRIPStat(g,rip,var)
                index = find(~isnan(g));g = g(index);
                XG = cell(size(g));
                for l=1:var.nrEl % building grouping variable
                    XG((g==var.El(l))) = {num2str(l)};
                end
                [F,~,FP] = SNP.ripAnova(XG,rip(index));
       end
       function [F,T,P] = ripAnova(XG,rip)
...
%            if isempty(obj.ParCont), out = []; return; end
%            if obj.ST3RedShape, out = obj.ParCont.RedShapeSpace; return; end 

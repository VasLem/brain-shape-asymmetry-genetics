classdef SNPv2 < superClassSNP
   ...
   properties (Hidden = true, Dependent = true)
      RegRuns; % Number of Regression runs (SNPv2BRIM)
      RegSampling; % Bootstrap/None (SNPv2BRIM)
      RegBalance;  % balance the data yes or no (SNPv2BRIM)
      RegBalanceTH; % desired balance factor (SNPv2BRIM)
      RegBalanceMethod; % UpSample/DownSample/ADASYN (SNPv2BRIM)
      OuterFold; % number of Outer Folds (SNPv2BRIM)
      InnerFold; % number of Inner Folds (SNPv2BRIM)
      UseRedShape; % Use reduced space or covariates (SNPv2BRIM)
      MinFoldSampleSize; % Min samples per Outer Fold and Inner Fold (SNPv2BRIM)
      MaxIterations; % Maximum number of BRIM iterations (SNPv2BRIM)
      Htest; % Perform Wilcoxon test om partial regression coefficients (SNPv2BRIM)
      RIPNormalize; % normalize RIP values based on group averages (SNPv2BRIM)
      StopCorr; % Stopping correlation between subsequent iterations (SNPv2BRIM)
      RIPStatPerm; % Number of permutations in testing significance of RIP values (SNPv2BRIM)
      GroupDef;% Definition of groups, GG = according to Test performed, GT = according to orginal genotypes (SNPv2BRIM)
      %ShapeSpace;
      %RedShapeSpace;
      %DepVar;
      %RedDepVar;
   ...
   methods % GENERAL: GETTING/SETTING
       function out = get.ParCont(obj)
           out = obj.ParCont;
           if ~superClassSNP.isH(out), out = []; end
       end
       ...
       function out = get.Balances(obj)
           ...
           out(1) = SNPv2.getBalance(obj.nrAA+obj.nrAB,obj.nrBB);% Recessive
           out(2) = SNPv2.getBalance(obj.nrBB+obj.nrAB,obj.nrAA);% Dominant
           out(3) = SNPv2.getBalance(obj.nrAA+obj.nrBB,obj.nrAB);% Over Dominant
           out(4) = SNPv2.getBalance(obj.nrAA,obj.nrBB);% AA versus BB
           out(5) = SNPv2.getBalance(obj.nrAA,obj.nrAB);% AA versus AB
           out(6) = SNPv2.getBalance(obj.nrAB,obj.nrBB);% AB versus BB
       end
   ...
   methods % STAGE 3: GETTING/SETTING
       ...
       function out = get.ST3PStrength(obj)
                out = SNPv2.pfast([obj.ST3FP obj.ST3GP]);
       end
   end
   methods % STAGE 4: GETTING/SETTING
       ...
       function out = get.ST4FFold(obj)
           if isempty(obj.ST4MOD), out = []; return; end
           out = obj.ST4MOD.FoldF;
       end
       function out = get.ST4PFold(obj)
           if isempty(obj.ST4MOD), out = []; return; end
           out = obj.ST4MOD.FoldFP;
       end
       function out = get.ST4F(obj)
           if isempty(obj.ST4MOD), out = []; return; end
           out = obj.ST4MOD.F;
       end
       function out = get.ST4P(obj)
           if isempty(obj.ST4MOD), out = []; return; end
           out = obj.ST4MOD.FP;
       end
   end
   methods % RUNNING STAGES
           ...
           for i=1:1:7
               try
                  ...
                  if ~(i==4) % TWO GROUP TESTS
                     [stat,p] = SNPv2.DStatTest(DepVar(GG==-1,:),DepVar(GG==1,:),t);  
                  else % THREE GROUP TEST / ADDITIVE
                     [stat,p] = SNPv2.FStatTest(DepVar(GG==-1,:),DepVar(GG==0,:),DepVar(GG==1,:),t);
                  end
       ...
       function obj = runStage2(obj,DepVar,parcont)
           ...
           for i=1:1:7
               try
                 out = SNPv2BRIM(obj,i,DepVar,[],Fouter);
       ...
       function obj = runStage3(obj,DepVar,Cov,parcont)
           if nargin==4, obj.ParCont = parcont;end
           try
               out = SNPv2BRIM(obj,obj.ST3TestInd(1),DepVar,Cov,[]);
       ...
       function obj = runStage4(obj,BaseCont,UseRedShape,parcont)
           try
               ...
               % Run SNPv2BRIM 
                 obj.ST4MOD = SNPv2BRIM(obj,obj.ST4TestInd,DepVar,Cov);
                 obj.ST4MOD = rmfield(obj.ST4MOD,{'nrAdded' 'DepVarAdd' 'IndVarAdd' 'GMAdd' 'BootIter' 'FoldG' 'FoldGP' 'DepVar' 'IndVar','Grp','H'});
               % Setting up reference scan 
                 RIPref = SNPv2.updateRIP(RefCoeff,obj.ST4MOD.M);
                 RIPref = SNPv2.normalizeRIP(RIPref,obj.ST4MOD.M,obj.ST4MOD.Var);
                 obj.ST4MOD.RIPRef = RIPref;
               % Complete population
                 %RIP = SNPv2.updateRIP(DepVar,obj.ST4MOD.M);
                 %RIP = SNPv2.normalizeRIP(RIP,obj.ST4MOD.M,obj.ST4MOD.Var);
                 %obj.ST4MOD.CompRIP = RIP;  
               % Population used to model things
                 RIP = SNPv2.updateRIP(DepVar(obj.ST4MOD.UsedInd,:),obj.ST4MOD.M);
                 RIP = SNPv2.normalizeRIP(RIP,obj.ST4MOD.M,obj.ST4MOD.Var);
                 obj.ST4MOD.RIP = RIP;
                 [obj.ST4MOD.RIPF,~,obj.ST4MOD.RIPG,~,obj.ST4MOD.RIPGT] = SNPv2.performRIPStat(obj.ST4MOD.GG,RIP,obj.ST4MOD.Var,0);
               ...
               % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     if GTInfo{i}.Nr>0
                        ...
                        [GTInfo{i}.AVG,GTInfo{i}.STD,GTInfo{i}.MED,GTInfo{i}.MAD,GTInfo{i}.O] = SNPv2.extractRobustRIPStatistics(GTInfo{i}.RIP,6);                     
                     end
                 end
   ...
   methods % Interfacing Functions
       ...
       function out = retrieveTestInd(obj,stage,stat)
           ...
           Tests = SNPv2.interpCodeP1(code(1:4));impos = SNPv2.interpCodeP2(code(5:7),P(5:7)>0.70);
...
%             obj.ST4MOD.GM = obj.ST4MOD.GM(find(ism));
   ...
   methods % NextGeneration BRIM
       function out = SNPv2BRIM(obj,TestInd,DepVar,Cov,Fouter)
            ...
            % Upsampling of groups if required
              ...
              if isempty(Fouter)% Cannot upsample with predefined outer folds
                  for i=1:1:grp.nrEl
                     ...
                     [DepVarTmp,IndVarTmp] = SNPv2.foldUpSample(DepVar(indG,:),IndVar(indG,:),nrSyn);
            ...
            % Outer Partitioning of Data
              ...
              parfor fo=1:ParInfo.OuterFold
                  ...
                  while ContBoot && Bootstrap>0
                    ...
                    % Upsampling of groups if required
                     ...
                     for i=1:1:grp.nrEl
                       ...
                       [DepVarTmp,IndVarTmp] = SNPv2.foldUpSample(FoTrDepVar(indG,:),FoTrIndVar(indG,:),nrSyn);
                   ...
                   % seperate data into inner folds
                     ...
                     TMPIndVar = FoTrIndVar(:,end);% Allocate Memory, only last collumn for SNPv2
                     for fi=1:ParInfo.InnerFold % Fi...
                         ...
                         rip = SNPv2.updateRIP(FoTrDepVar(FiTestInd,:),M)';% Get RIP scores
                         if ParInfo.RIPNormalize, rip = SNPv2.normalizeRIP(rip,M,var);end% rescale RIP scores
                 ...
                 % Evaluate outer test data
                  ...
                  [FoldResults{fo}.F,FoldResults{fo}.FP,FoldResults{fo}.G,FoldResults{fo}.GP] = SNPv2.performRIPStat(FoldResults{fo}.IndVar,FoldResults{fo}.RIP,var,ParInfo.RIPStatPerm);
              end
             % Gathering Fold Results
               ...
               index = find(~isnan(out.FoldFP));
               out.FP = SNPv2.pfast(out.FoldFP(index));out.F = nanmedian(FoldF);
               out.FoldG = FoldG;out.FoldGP = FoldGP;out.FoldGP(out.FoldGP==0) = 0.001/ParInfo.RIPStatPerm;
               out.GP = SNPv2.pfast(out.FoldGP(index));out.G = nanmedian(FoldG);
               out.BootIter = BootIter;
              % Extracting final M 
                out.M = median(M);
                if ~ParInfo.Htest, return; end
                if ParInfo.OuterFold>=8, 
                   out.H = SNPv2.wilcoxonM(M);
                else
                   out.H = ones(1,dim); 
                end
                out.M = out.H.*out.M;
       ...
       function [M,H,Mfor] = robustPLSRegression(obj,indvar,depvar,gg,parinfo)
           ...
           % execute runs
             for s=1:1:runs
                 if balance % Balance the given input
                    if upsampling % Balancing by Increasing Minority Groups
                        ...
                        for i=1:1:nrgg
                            ...
                            [DepVarNew,IndVarNew] = SNPv2.regUpSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i});
                        ...
                        for i=1:1:nrgg
                            [DepVarNew,IndVarNew] = SNPv2.regDownSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i}.nrKeep);
                            SampleDepVar = [SampleDepVar; DepVarNew];
                            SampleIndVar = [SampleIndVar; IndVarNew];
                        end
                    end
                 ...
                 Mfor(s,:) = SNPv2.getRegression(SampleIndVar(SampleInd,:),SampleDepVar(SampleInd,:));
             end
           % finalizing output
             ...
             H = SNPv2.wilcoxonM(Mfor);% wilcoxon test for median
       end
   end
   methods % Model functions
       ...
       function [C,Atypicality,scan] = overLaySNPv2(obj,BaseCont,genotype,Ref,BF,maxAtyp,adjust,TrimBF)
                ...
                if GTInfo.Nr==0, return; end
                Xref = SNPv2.updateRIP(Ref.Coeff,obj.ST4MOD.M);
                Xref = SNPv2.normalizeRIP(Xref,obj.ST4MOD.M,obj.ST4MOD.Var);
       ...
       function [IB,pTIB,pT] = matchFaces(obj,faces,genotype,BF)
                rip = SNPv2.updateRIP(faces,obj.ST4MOD.M);
                rip = SNPv2.normalizeRIP(rip,obj.ST4MOD.M,obj.ST4MOD.Var);
                ...
                % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     if obj.ST4MOD.GTInfo{i}.Nr>0
                        ...
                        [obj.ST4MOD.GTInfo{i}.AVG,obj.ST4MOD.GTInfo{i}.STD,obj.ST4MOD.GTInfo{i}.MED,obj.ST4MOD.GTInfo{i}.MAD,GTInfo{i}.O] = SNPv2.extractRobustRIPStatistics(obj.ST4MOD.GTInfo{i}.RIP,kappa);                     
                     end
                 end
                % Creating average morphs
                 obj.ST4MOD.Atyps = nan*zeros(1,3);
   ...
   methods (Static = true)
       ...
       function [F,FP,Gmean,GmeanP,GT,FPar] = performRIPStat(g,rip,var,t)
                F = nan;FP = nan;Gmean = nan;GmeanP = nan;GT = nan;
                ...
                [F,FPar] = SNPv2.ripAnova(XG,rip);
                if isnan(F), return; end
                if doroc, [Gmean,GT] = SNPv2.ripROC(g,rip,var.PosClass); end
                if t == 0, return;end
                if doroc, GCount = zeros(1,t); end
                FCount = zeros(1,t);
                parfor i=1:t
                    ind = randperm(n);
                    Ffor = SNPv2.ripAnova(XG,rip(ind));
                    FCount(i) = Ffor>=F;
                    if doroc, Gmeanfor = SNPv2.ripROC(g,rip(ind),var.PosClass); GCount(i) = Gmeanfor>=Gmean; end
                end
                FP = sum(FCount)/t;

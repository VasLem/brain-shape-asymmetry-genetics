classdef USNPMOD < superClassSNP
   % General Properties
   properties
      RS = [];
      GT = [];
   ...
   methods % GETTING/SETTING
       ...
       function out = get.GGGTBal(obj)
          if isempty(obj.GT), out = []; return; end
          out = USNPMOD.getGGGTBalance(obj.GG,obj.GT);
       end
       ...
       function out = get.Balances(obj)
           ...
           out(1) = USNPMOD.getBalance(obj.nAA+obj.nAB,obj.nBB);% Recessive
           out(2) = USNPMOD.getBalance(obj.nBB+obj.nAB,obj.nAA);% Dominant
           out(3) = USNPMOD.getBalance(obj.nAA+obj.nBB,obj.nAB);% Over Dominant
           out(4) = USNPMOD.getBalance(obj.nAA,obj.nBB);% AA versus BB
           out(5) = USNPMOD.getBalance(obj.nAA,obj.nAB);% AA versus AB
           out(6) = USNPMOD.getBalance(obj.nAB,obj.nBB);% AB versus BB
       end
   ...
   methods % SNP ANALYSIS
       function out = runR2Test(obj,DepVar,COV,t)
           ...
           [out.R2,out.pR2] = USNPMOD.testPartialPLSR(GT2GG(obj,obj.GT),DepVar,COV,t);
       ...
       function out = runAngleTest(obj,DepVar,COV,t,maxM)
           ...
           for i=1:t
               ...
               for k=1:1:K
                   ind = find(F==k);
                   forM(:,k) = USNPMOD.getRegression([COV(ind,:) GG(ind)],DepVar(ind,:));
               end
       ...
       function [LocD,LocP] = locationTest(obj,t,DepVar)
           GG = GT2GG(obj,obj.GT);
           if ~(obj.TestInd==4)
              [LocD,LocP] = USNPMOD.DStatTest(DepVar(GG==-1,:),DepVar(GG==1,:),t); 
           else
              [LocD,LocP] = USNPMOD.FStatTest(DepVar(GG==-1,:),DepVar(GG==0,:),DepVar(GG==1,:),t);
           end
       end
   end
   methods % MODEL BUILDING
       function buildFromAngleTest(obj,DepVar,COV,TestA)
          ...
          % INITIALIZING 
           ...
           out.UsedInd = ind;
           GG = GG(ind);
           out.GG = GG;out.GT = obj.GT(ind);out.GGGTBal = USNPMOD.getGGGTBalance(out.GG,out.GT);
           DepVar = DepVar(ind,:);
          % VAR INFORMATION 
          ...
          % M CONSTRUCTION
           ...
           out.H = USNPMOD.wilcoxonM(fullM');
           out.HM = out.M.*out.H;
          % RIP COMPUTATIONS
           out.MRIP = USNPMOD.updateRIP(DepVar,out.M)';
           out.NormMRIP = USNPMOD.normalizeRIP(out.MRIP,out.M,out.Var);
           out.HMRIP = USNPMOD.updateRIP(DepVar,out.HM)';
           out.NormHMRIP = USNPMOD.normalizeRIP(out.HMRIP,out.HM,out.Var);
          % RIP STAT
           %[F,FP,~,Gmean,~,~] = USNPMOD.performRIPStat(out.GG,out.MRIP,out.Var,0);
          % FINAL OUTPUT
           %[out.GGInfo,out.Freq] = USNPMOD.getGTInfo(GG,RIP);
           [out.Balance,out.MinBalance] = USNPMOD.getGGBalance(GG);
           obj.MOD = out;
       end
       function build(obj,DepVar,COV)
               ...
               % Retrieving GT Info and statistics
                 [obj.MOD.GGInfo,obj.MOD.Freq] = USNPMOD.getGTInfo(obj.MOD.GG,obj.MOD.RIP);  
       end
            ...
            % Outer Partitioning of Data
              ...
              parfor fo=1:obj.OuterFold
                  ...
                  while ContBoot && Bootstrap>0
                   ...
                   % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         rip = USNPMOD.updateRIP(FoTrDepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = USNPMOD.normalizeRIP(rip,M,var);end% rescale RIP scores
                 ...
                 % Evaluate outer test data
                  ...
                  [FoldResults{fo}.F,FoldResults{fo}.FP,FoldResults{fo}.FPar,FoldResults{fo}.G,FoldResults{fo}.GP,~] = USNPMOD.performRIPStat(FoldResults{fo}.GG,FoldResults{fo}.RIP,var,obj.RIPStatPerm);
             ...
             % Gathering Fold Results
               ...
               if ~isempty(index), out.FPar = USNPMOD.pfast(FoldFPar); end
               out.FP = nan;
               out.GP = nan;
               if obj.RIPStatPerm>0
                  FoldFP(FoldFP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldFP));
                  if ~isempty(index), out.FP = USNPMOD.pfast(FoldFP(index)); end
                  FoldGP(FoldGP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldGP));
                  if ~isempty(index), out.GP = USNPMOD.pfast(FoldGP(index)); end
               end
               out.FoldF = FoldF;
               out.FoldG = FoldG;
              ...
              % Extracting final M 
               ...
               if obj.OuterFold>=8, 
                  out.H = USNPMOD.wilcoxonM(M);
               else
                  out.H = ones(1,dim); 
               end
               out.M = out.H.*out.M;
               RIP = USNPMOD.updateRIP(DepVar,out.M);
               if obj.RIPNormalize, RIP = USNPMOD.normalizeRIP(RIP,out.M,var); end
               out.RIP = RIP;
               [out.RIPF,out.RIPFP,out.RIPFPar,out.RIPG,out.RIPGP,~] = USNPMOD.performRIPStat(out.GG,RIP,var,0);
              ...
              while ContBoot && Bootstrap>0
                    ...
                    % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         if obj.Htest, M = H.*M;end% Only keep significant regression coefficients
                         rip = USNPMOD.updateRIP(DepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = USNPMOD.normalizeRIP(rip,M,var);end% rescale RIP scores
                         TMPIndVar(FiTestInd) = rip;% Store updated RIP scores, always in the last collumn
                     end
                   % monitoring progress of Booting
       ...
       function out = PLSR(obj,DepVar,Cov)
           ...
           % Defining Independent and Dependent Variables
            ...
            % Extracting final M from outer training data
              ...
              out.M = M;out.H = H;
              RIP = USNPMOD.updateRIP(DepVar,out.M);
              if obj.RIPNorm, RIP = USNPMOD.normalizeRIP(RIP,out.M,var);end
              out.RIP = RIP;
              [out.RIPF,out.RIPFP,out.RIPFPar,out.RIPG,out.RIPGP,~] = USNPMOD.performRIPStat(out.GG,RIP,var,0);  
       end
       function [M,H,Mfor] = robustPLSRegression(obj,indvar,depvar,gg,perc)
           ...
           % execute runs
             for s=1:1:runs
                 if balance % Balance the given input
                    if upsampling % Balancing by Increasing Minority Groups
                        ...
                        for i=1:1:nrgg
                            ...
                            [DepVarNew,IndVarNew] = USNPMOD.regUpSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i});
                        ...
                        for i=1:1:nrgg
                            [DepVarNew,IndVarNew] = USNPMOD.regDownSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i}.nrKeep);
                 ...
                 Mfor(s,:) = USNPMOD.getRegression(SampleIndVar(SampleInd,:),SampleDepVar(SampleInd,:));
             end
           % finalizing output
             ...
             H = USNPMOD.wilcoxonM(Mfor);% wilcoxon test for median
       end       
   ...
   methods % BIOMETRICS
       function [EER,G,AUC,R] = oppGGBiometric(obj,trgg,trrip,gg,rip,posclass)
                ...
                [EER,G,AUC] = USNPMOD.getEER(ratio(indexp),ratio(indexn));
                R = USNPMOD.getRANK(ratio(indexp),ratio(indexn));
       end
   end
   ...
   methods % INTERFACING     
       ...
       function [Distr,Freq,RIP] = GT2RIP(obj,in)
           ...
           [Distr,Freq,RIP] = USNPMOD.GG2RIP(obj.MOD.GG,getMODRIP(obj,obj.MOD),gg);
       end
       ...
       function out = USNPMOD2SNPMOD(obj,AngleTable)
                out = SNPMOD;
                out.X = obj.GT;
                out.RS = obj.RS;
                out.TestInd = obj.TestInd;
                out.Level = obj.Level;
                out.Cluster = obj.Cluster;
                out.M = obj.MOD.M;
                out.H = obj.MOD.H;
                out.HM = obj.MOD.HM;
                out.MRIP = obj.MOD.MRIP;
                out.NormMRIP = obj.MOD.NormMRIP;
                out.HMRIP = obj.MOD.HMRIP;
                out.NormHMRIP = obj.MOD.NormHMRIP;
                out.VarInfo = obj.MOD.Var;
                out.VarInfo = rmfield(out.VarInfo,{'Mindex' 'Pindex'});
                out.A = obj.A;
                out.pA = obj.pA;
                out.R2 = obj.R2;
                out.pR2 = obj.pR2;
                out.MODBal = obj.MOD.Balance;
                out.MODMinBal = obj.MOD.MinBalance;
                if nargin < 2, return; end
                ShapeDim = length(out.M);
                out.pAR = 1;
                out.pAR = PPMMOD.lookUppRA(obj.A,ShapeDim,AngleTable);
       end
   end
   methods (Static = true)
       ...
       function [F,FP,FPar,Gmean,GmeanP,GT] = performRIPStat(g,rip,var,t)
                ...
                [F,FPar] = USNPMOD.ripAnova(XG,rip);
                if isnan(F), return; end % BAD TEST GROUP
                if doroc, [Gmean,GT] = USNPMOD.ripROC(g,rip,var.PosClass); end
                if t == 0,return;end
                FCount = zeros(1,t);
                if doroc, GCount = zeros(1,t); end
                parfor i=1:t
                    ind = randperm(n);
                    Ffor = USNPMOD.ripAnova(XG,rip(ind));
                    FCount(i) = Ffor>=F;
                    if doroc, Gmeanfor = USNPMOD.ripROC(g,rip(ind),var.PosClass); GCount(i) = Gmeanfor>=Gmean; end
                end
                FP = sum(FCount)/t;
       ...
       function [Soft,Hard] = evalQD(trgg,trrip,gg,rip,posclass,t)
                ...
                [Soft.EER,Soft.G,Soft.AUC] = USNPMOD.getEER(tmatches,fmatches);
                Soft.R = USNPMOD.getRANK(tmatches,fmatches);
                % HARD ASSESSMENT
                [Hard.PREC,Hard.REC,Hard.G,Hard.TNF] = USNPMOD.getHardClass(ratio,indexp,indexn,1);
                if t==0, return; end
                ...
                parfor i=1:t
                   ...
                   [forEER,~,forAUC] = USNPMOD.getEER(fortmatches,forfmatches);
                   tmp = USNPMOD.getRANK(fortmatches,forfmatches);
                   forR = tmp(4);
                   EERcount(i) = forEER<=Soft.EER;
                   AUCcount(i) = forAUC>=Soft.AUC;
                   Rcount(i) = forR<=Soft.R(4);
                   [~,~,forG] = USNPMOD.getHardClass(forratio,indexp,indexn,1);
       ...
       function [out,freq] = getGTInfo(gg,rip)
              % Obtaining group memberships and additional needed info
                 ...
                 for i=1:1:nrGG
                     ...
                     [out{i}.AVG,out{i}.STD,out{i}.MED,out{i}.MAD] = USNPMOD.extractRIPStatistics(rip(out{i}.Ind));
       ...
       function [Distr,Freq,RIP] = GG2RIP(trGG,trrip,GG)
           ...
           [Distr.Tmu,Distr.Tsigma,Distr.TO] = USNPMOD.extractRIPStatistics(trrip(Tind)');
           RIP = Distr.Tmu;Distr.TpMax = normpdf(Distr.Tmu,Distr.Tmu,Distr.Tsigma);
           [Distr.Fmu,Distr.Fsigma,Distr.FO] = USNPMOD.extractRIPStatistics(trrip(Find)');
...
%            RIP = USNPMOD.updateRIP(DepVar,out.M)';
%            if obj.RIPNorm, RIP = USNPMOD.normalizeRIP(RIP,out.M,out.Var);end
%            out.RIP = RIP;
%            [out.GGInfo,out.Freq] = USNPMOD.getGTInfo(GG,RIP);
%            [out.Balance,out.MinBalance] = USNPMOD.getGGBalance(GG);
%            obj.MOD = out;

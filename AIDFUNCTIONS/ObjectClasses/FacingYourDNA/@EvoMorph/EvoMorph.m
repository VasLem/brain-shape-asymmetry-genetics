classdef EvoMorph < superClass
    ...
    properties (Transient = true, Hidden = true)
       ...
       GT = [];            % Genotypes
       GTAtyp = [];        % Atypicality of Genotypes
       Cov = [];           % Covariates
    ...
    properties 
       ...
       WeightMatches = 'NO'; % Weighting the individual scores of matches to SNP models, NO, Atyp
    ...
    methods % CONSTRUCTOR
        function obj = EvoMorph(varargin)
    ...
    methods % AID INTERFACE FUNCTIONS
        ...
        function out = evalSolutions(obj,sol,pSol)
            n = size(sol,1);
            if obj.UseRedShape
               BaseMatch = -1*ones(1,n);
            else
               BaseMatch = matchFaces(obj.BaseCont,sol,obj.CovDistr,obj.GBDistr); 
            end
            SNPMatch = matchFaces(obj.SNPCont,sol,obj.GT,obj.BF);
            out = (1-obj.FacePenalty).*nanmean(repmat(obj.MatchW,1,n).*-log([BaseMatch;SNPMatch])) + obj.FacePenalty*(pSol);
        end

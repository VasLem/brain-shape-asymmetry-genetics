classdef SNPClassify < superClassSNP
   % General Properties
   properties
      RS = [];
      GT = [];
   ...
   methods % CONSTRUCTOR
        function obj = SNPClassify(varargin)
   ...
   methods % GETTING/SETTING
       ...
       function out = get.Balances(obj)
           ...
           out(1) = SNPClassify.getBalance(obj.nrAA+obj.nrAB,obj.nrBB);% Recessive
           out(2) = SNPClassify.getBalance(obj.nrBB+obj.nrAB,obj.nrAA);% Dominant
           out(3) = SNPClassify.getBalance(obj.nrAA+obj.nrBB,obj.nrAB);% Over Dominant
           out(4) = SNPClassify.getBalance(obj.nrAA,obj.nrBB);% AA versus BB
           out(5) = SNPClassify.getBalance(obj.nrAA,obj.nrAB);% AA versus AB
           out(6) = SNPClassify.getBalance(obj.nrAB,obj.nrBB);% AB versus BB
       end
   ...
   methods % MODEL BUILDING
       ...
       function [LocD,LocP] = locationTest(obj,t,DepVar)
           ...
           if ~(obj.TestInd==4)
              [LocD,LocP] = SNPClassify.DStatTest(DepVar(GG==-1,:),DepVar(GG==1,:),t); 
           else
              [LocD,LocP] = SNPClassify.FStatTest(DepVar(GG==-1,:),DepVar(GG==0,:),DepVar(GG==1,:),t);
           end
       end
       function build(obj,DepVar,Cov)
               ...
               % Setting up reference scan 
                 %RIPref = SNPClassify.updateRIP(BaseCont.ST4Ref.Coeff,obj.MOD.M);
                 %if obj.RIPNormalize, RIPref = SNPClassify.normalizeRIP(RIPref,obj.MOD.M,obj.MOD.Var);end
                 %obj.MOD.RIPRef = RIPref;
               % Retrieving GT Info and statistics
                 [obj.MOD.GTInfo,obj.MOD.Freq] = SNPClassify.getGTInfo(obj.TestInd,obj.MOD.GG,obj.MOD.RIP);  
       end
       function crossValidate(obj,DepVar,Cov,K,t,ExtFaces,ExtGT)
                n = size(DepVar,1);
                if isscalar(K)
                ...
                for k=1:1:K
                    ...
                    rip = SNP.updateRIP(TestFaces,Fold{k}.MOD.M);
                    if Fold{k}.RIPNormalize, rip = SNP.normalizeRIP(rip,Fold{k}.MOD.M,Fold{k}.MOD.Var);end
                    index = find(~isnan(gg));
                    [Soft,Hard] = SNPClassify.evalQD(Fold{k}.MOD.GG,Fold{k}.MOD.RIP,gg(index),rip(index)',Fold{k}.MOD.Var.PosClass,t);   
                    EER(1,k) = Soft.EER;EER(2,k) = Soft.pEER;
                    AUC(1,k) = Soft.AUC;AUC(2,k) = Soft.pAUC;
                    Gs(1,k) = Soft.G;Gs(2,k) = Soft.pEER;
                    R(:,k) = Soft.R';
                    PREC(k) = Hard.PREC;
                    REC(k) = Hard.REC;
                    Gh(1,k) = Hard.G;
                ...
                p = obj.CV.EER(2,:);
                p(find(p==0)) = 0.1/t;
                p = p(find(~isnan(obj.CV.EER(1,:))));
                if ~isempty(p), obj.CV.pEER = SNPClassify.pfast(p); end
                if nargin < 5; return; end% no external test set is given
                EER = nan*zeros(2,K);
                AUC = nan*zeros(2,K);
                Gs = nan*zeros(2,K);
                R = nan*zeros(4,K);
                ...
                for k=1:1:K
                    ...
                    rip = SNP.updateRIP(ExtFaces,Fold{k}.MOD.M);
                    rip = SNP.normalizeRIP(rip,Fold{k}.MOD.M,Fold{k}.MOD.Var);
                    index = find(~isnan(gg));
                    [Soft,Hard] = SNPClassify.evalQD(Fold{k}.MOD.GG,Fold{k}.MOD.RIP,gg(index),rip(index)',Fold{k}.MOD.Var.PosClass,t);   
                    EER(1,k) = Soft.EER;EER(2,k) = Soft.pEER;
                    AUC(1,k) = Soft.AUC;AUC(2,k) = Soft.pAUC;
                ...
                p = obj.EV.EER(2,:);
                p(find(p==0)) = 0.1/t;
                p = p(find(~isnan(obj.EV.EER(1,:))));
                if ~isempty(p), obj.EV.pEER = SNPClassify.pfast(p); end
       end
            ...
            % Outer Partitioning of Data
              ...
              parfor fo=1:obj.OuterFold
                  ...
                  while ContBoot && Bootstrap>0
                   ...
                   % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         rip = SNPClassify.updateRIP(FoTrDepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = SNPClassify.normalizeRIP(rip,M,var);end% rescale RIP scores
                 ...
                 % Evaluate outer test data
                  ...
                  [FoldResults{fo}.F,FoldResults{fo}.FP,FoldResults{fo}.FPar,FoldResults{fo}.G,FoldResults{fo}.GP,~] = SNPClassify.performRIPStat(FoldResults{fo}.GG,FoldResults{fo}.RIP,var,obj.RIPStatPerm);
             ...
             % Gathering Fold Results
               ...
               if ~isempty(index), out.FPar = SNPClassify.pfast(FoldFPar); end
               out.FP = nan;
               out.GP = nan;
               if obj.RIPStatPerm>0
                  FoldFP(FoldFP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldFP));
                  if ~isempty(index), out.FP = SNPClassify.pfast(FoldFP(index)); end
                  FoldGP(FoldGP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldGP));
                  if ~isempty(index), out.GP = SNPClassify.pfast(FoldGP(index)); end
               end
               out.FoldF = FoldF;
               out.FoldG = FoldG;
              ...
              % Extracting final M 
               ...
               if obj.OuterFold>=8, 
                  out.H = SNPClassify.wilcoxonM(M);
               else
                  out.H = ones(1,dim); 
               end
               out.M = out.H.*out.M;
               RIP = SNPClassify.updateRIP(DepVar,out.M);
               if obj.RIPNormalize, RIP = SNPClassify.normalizeRIP(RIP,out.M,var); end
               out.RIP = RIP;
               [out.RIPF,out.RIPFP,out.RIPFPar,out.RIPG,out.RIPGP,~] = SNPClassify.performRIPStat(out.GG,RIP,var,0);
              ...
              while ContBoot && Bootstrap>0
                    ...
                    % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         if obj.Htest, M = H.*M;end% Only keep significant regression coefficients
                         rip = SNPClassify.updateRIP(DepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = SNPClassify.normalizeRIP(rip,M,var);end% rescale RIP scores
                         TMPIndVar(FiTestInd) = rip;% Store updated RIP scores, always in the last collumn
                     end
                   % monitoring progress of Booting
       ...
       function [M,H,Mfor] = robustPLSRegression(obj,indvar,depvar,gg,perc)
           ...
           % execute runs
             for s=1:1:runs
                 if balance % Balance the given input
                    if upsampling % Balancing by Increasing Minority Groups
                        ...
                        for i=1:1:nrgg
                            ...
                            [DepVarNew,IndVarNew] = SNPClassify.regUpSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i});
                        ...
                        for i=1:1:nrgg
                            [DepVarNew,IndVarNew] = SNPClassify.regDownSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i}.nrKeep);
                 ...
                 Mfor(s,:) = SNPClassify.getRegression(SampleIndVar(SampleInd,:),SampleDepVar(SampleInd,:));
             end
           % finalizing output
             ...
             H = SNPClassify.wilcoxonM(Mfor);% wilcoxon test for median
       end       
       ...
       function out = biometricEval(obj,GT,Faces,t)
                if nargin < 4, t = 0; end
                gg = getGG(obj,GT);
                rip = SNP.updateRIP(Faces,obj.MOD.M);
                if obj.RIPNormalize, rip = SNP.normalizeRIP(rip,obj.MOD.M,obj.MOD.Var); end
                index = find(~isnan(gg));
                [out.Soft,out.Hard] = SNPClassify.evalQD(obj.MOD.GG,obj.MOD.RIP,gg(index),rip(index)',obj.MOD.Var.PosClass,t);
       end
   end
   methods % MODEL EVALUATION
       function foldEvaluation(obj,t)
           ...
           for f=1:1:nrF
               [obj.EvalEER(f),obj.EvalG(f),obj.EvalR(f),obj.EvalpEER(f),obj.EvalpG(f),obj.EvalpR(f)] = SNPClassify.evalQD(obj.Folds{f}.TrGG,obj.Folds{f}.TrRIP,obj.Folds{f}.GG,obj.Folds{f}.RIP,obj.MOD.Var.PosClass,t);
           end
           if t==0, return; end
           obj.EvalpEER(find(obj.EvalpEER==0)) = 0.1/t;
           obj.EvalpG(find(obj.EvalpG==0)) = 0.1/t;
           obj.EvalpR(find(obj.EvalpR==0)) = 0.1/t;
           obj.EvalFishEER = SNPClassify.pfast(obj.EvalpEER);
           obj.EvalFishG = SNPClassify.pfast(obj.EvalpG);
           obj.EvalFishR = SNPClassify.pfast(obj.EvalpR);
       end  
   end
   methods % MODEL APPLICATION
       ...
       function [C,Atypicality,scan] = overLaySNPClassify(obj,BaseCont,genotype,Ref,BF,maxAtyp,adjust,TrimBF)
                ...
                if GTInfo.Nr==0, return; end
                Xref = SNPClassify.updateRIP(Ref.Coeff,obj.MOD.M);
                Xref = SNPClassify.normalizeRIP(Xref,obj.MOD.M,obj.MOD.Var);
       ...
       function [IB,pTIB,pT] = matchFaces(obj,faces,genotype,BF)
                rip = SNPClassify.updateRIP(faces,obj.MOD.M);
                rip = SNPClassify.normalizeRIP(rip,obj.MOD.M,obj.MOD.Var);
                ...
                % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     if obj.MOD.GTInfo{i}.Nr>0
                        ...
                        [obj.MOD.GTInfo{i}.AVG,obj.MOD.GTInfo{i}.STD,obj.MOD.GTInfo{i}.MED,obj.MOD.GTInfo{i}.MAD,GTInfo{i}.O] = SNPClassify.extractRobustRIPStatistics(obj.MOD.GTInfo{i}.RIP,kappa);                     
                     end
                 end
   ...
   methods (Static = true)
       ...
       function [F,FP,FPar,Gmean,GmeanP,GT] = performRIPStat(g,rip,var,t)
                ...
                [F,FPar] = SNPClassify.ripAnova(XG,rip);
                if isnan(F), return; end % BAD TEST GROUP
                if doroc, [Gmean,GT] = SNPClassify.ripROC(g,rip,var.PosClass); end
                if t == 0,return;end
                FCount = zeros(1,t);
                if doroc, GCount = zeros(1,t); end
                parfor i=1:t
                    ind = randperm(n);
                    Ffor = SNPClassify.ripAnova(XG,rip(ind));
                    FCount(i) = Ffor>=F;
                    if doroc, Gmeanfor = SNPClassify.ripROC(g,rip(ind),var.PosClass); GCount(i) = Gmeanfor>=Gmean; end
                end
                FP = sum(FCount)/t;
       ...
       function [Soft,Hard] = evalQD(trgg,trrip,gg,rip,posclass,t)
                ...
                [Soft.EER,Soft.G,Soft.AUC] = SNPClassify.getEER(tmatches,fmatches);
                Soft.R = SNPClassify.getRANK(tmatches,fmatches);
                % HARD ASSESSMENT
                [Hard.PREC,Hard.REC,Hard.G,Hard.TNF] = SNPClassify.getHardClass(ratio,indexp,indexn,1);
                if t==0, return; end
                ...
                for i=1:t
                   ...
                   [forEER,~,forAUC] = SNPClassify.getEER(fortmatches,forfmatches);
                   tmp = SNPClassify.getRANK(fortmatches,forfmatches);
                   forR = tmp(4);
                   EERcount(i) = forEER<=Soft.EER;
                   AUCcount(i) = forAUC>=Soft.AUC;
                   Rcount(i) = forR<=Soft.R(4);
                   [~,~,forG] = SNPClassify.getHardClass(forratio,indexp,indexn,1);
           ...
           % Obtaining group memberships and additional needed info
               ...
               % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     [out{i}.AVG,out{i}.STD,out{i}.MED,out{i}.MAD] = SNPClassify.extractRIPStatistics(rip(out{i}.Ind));                     
                 end
...
%                 end
%                 [EER] = SNPClassify.getEER(-log(ratio),-log(ratio.^-1));
%                 
%                 indexp = find(gg==posclass);
%                 [EERP] = SNPClassify.getEER(-log(ratio(indexp)),-log(ratio(indexp).^-1));
%                 indexn = setdiff(1:n,indexp);
%                 [EERN] = SNPClassify.getEER(-log(ratio(indexn)),-log(ratio(indexn).^-1));
%                 
%                 
%                 if t==0, return; end
%                 count = zeros(1,t);
...
%                    end
%                    [forEER] = SNPClassify.getEER(-log(forratio),-log(forratio.^-1)); 
%                    count(i) = forEER<=EER;
%                 end
%                 P = sum(count)/t;
%        end

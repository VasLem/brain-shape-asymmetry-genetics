classdef G2FPredictionV4 < superClass
   ...
   methods % GENERAL GETTING\SETTING
        ...
        function out = get.AvgFreq(obj)
            if isempty(obj.GTFreq), out = nan; return; end
            out = nanmean(obj.GTFreq);
        end
        function out = get.AvgAtyp(obj)
            if isempty(obj.GTAtyp), out = nan; return; end
            out = nanmean(obj.GTAtyp);
        end
   ...
   methods % SNP OVERLAYING GETTING\SETTING
       function out = get.LinkedGT(obj)
          if obj.Testing, out = obj.GT; return; end
          out = []; 
   ...
   methods % SUB INTERFACE FUNCTIONS
       ...
       function linkWithSNPCont(obj,SNPCont)
                ...
                if ~obj.Testing, [obj.SNPExtract,obj.SNPPresent] = vlookup(getRS(SNPCont),obj.RS,false);end
                ...
                obj.GTFreq = getGTFreq(SNPCont,obj.LinkedGT);
                %disp('----------------------------------------------------------------------------');
       end

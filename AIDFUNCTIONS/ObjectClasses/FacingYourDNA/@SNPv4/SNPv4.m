classdef SNPv4 < superClassSNP
   ...
   methods % GETTING/SETTING
       ...
       function out = get.Balances(obj)
           ...
           out(1) = SNPv4.getBalance(obj.nrAA+obj.nrAB,obj.nrBB);% Recessive
           out(2) = SNPv4.getBalance(obj.nrBB+obj.nrAB,obj.nrAA);% Dominant
           out(3) = SNPv4.getBalance(obj.nrAA+obj.nrBB,obj.nrAB);% Over Dominant
           out(4) = SNPv4.getBalance(obj.nrAA,obj.nrBB);% AA versus BB
           out(5) = SNPv4.getBalance(obj.nrAA,obj.nrAB);% AA versus AB
           out(6) = SNPv4.getBalance(obj.nrAB,obj.nrBB);% AB versus BB
       end
   ...
   methods % MODEL BUILDING
       ...
       function [LocD,LocP] = locationTest(obj,t,DepVar)
           ...
           if ~(obj.TestInd==4)
              [LocD,LocP] = SNPv4.DStatTest(DepVar(GG==-1,:),DepVar(GG==1,:),t); 
           else
              [LocD,LocP] = SNPv4.FStatTest(DepVar(GG==-1,:),DepVar(GG==0,:),DepVar(GG==1,:),t);
           end
       end
       function build(obj,DepVar,Cov)
               ...
               % Retrieving GT Info and statistics
                 [obj.MOD.GTInfo,obj.MOD.Freq] = SNPv4.getGTInfo(obj.TestInd,obj.MOD.GG,obj.MOD.RIP);  
       end
       function [CV,EV] = crossValidate(obj,DepVar,Cov,K1,K,FD,CL,t,ExtFaces,ExtGT)
                ...
                for k1=1:1:K1
                    ...
                    for k=1:1:K
                        ...
                        rip = SNPv4.updateRIP(TestFaces,Fold{k}.MOD.M);
                        if Fold{k}.RIPNormalize, rip = SNPv4.normalizeRIP(rip,Fold{k}.MOD.M,Fold{k}.MOD.Var);end
                        index = find(~isnan(gg));
                        [Soft,Hard] = SNPv4.evalQD(Fold{k}.MOD.GG,Fold{k}.MOD.RIP,gg(index),rip(index)',Fold{k}.MOD.Var.PosClass,t);   
                    ...
                    for k=1:1:K
                        ...
                        rip = SNPv4.updateRIP(ExtFaces,Fold{k}.MOD.M);
                        if Fold{k}.RIPNormalize,rip = SNPv4.normalizeRIP(rip,Fold{k}.MOD.M,Fold{k}.MOD.Var);end
                        index = find(~isnan(gg));
                        [Soft,Hard] = SNPv4.evalQD(Fold{k}.MOD.GG,Fold{k}.MOD.RIP,gg(index),rip(index)',Fold{k}.MOD.Var.PosClass,t);   
       ...
       function [CV] = crossValidatev2(obj,DepVar,Cov,K1,K,FD,CL,t)
                ...
                parfor k1=1:1:K1
                    ...
                    for k=1:1:K
                        ...
                        if Fold{k}.RIPNormalize, rip = SNPv4.normalizeRIP(rip,Fold{k}.MOD.M,Fold{k}.MOD.Var);end
                        index = find(~isnan(gg));
                        Soft = SNPv4.evalQD(Fold{k}.MOD.GG,Fold{k}.MOD.RIP,gg(index),rip(index)',Fold{k}.MOD.Var.PosClass,t);   
            ...
            % Outer Partitioning of Data
              ...
              parfor fo=1:obj.OuterFold
                  ...
                  while ContBoot && Bootstrap>0
                   ...
                   % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         rip = SNPv4.updateRIP(FoTrDepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = SNPv4.normalizeRIP(rip,M,var);end% rescale RIP scores
                 ...
                 % Evaluate outer test data
                  ...
                  [FoldResults{fo}.F,FoldResults{fo}.FP,FoldResults{fo}.FPar,FoldResults{fo}.G,FoldResults{fo}.GP,~] = SNPv4.performRIPStat(FoldResults{fo}.GG,FoldResults{fo}.RIP,var,obj.RIPStatPerm);
             ...
             % Gathering Fold Results
               ...
               if ~isempty(index), out.FPar = SNPv4.pfast(FoldFPar); end
               out.FP = nan;
               out.GP = nan;
               if obj.RIPStatPerm>0
                  FoldFP(FoldFP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldFP));
                  if ~isempty(index), out.FP = SNPv4.pfast(FoldFP(index)); end
                  FoldGP(FoldGP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldGP));
                  if ~isempty(index), out.GP = SNPv4.pfast(FoldGP(index)); end
               end
               out.FoldF = FoldF;
               out.FoldG = FoldG;
              ...
              % Extracting final M 
               ...
               if obj.OuterFold>=8, 
                  out.H = SNPv4.wilcoxonM(M);
               else
                  out.H = ones(1,dim); 
               end
               out.M = out.H.*out.M;
               RIP = SNPv4.updateRIP(DepVar,out.M);
               if obj.RIPNormalize, RIP = SNPv4.normalizeRIP(RIP,out.M,var); end
               out.RIP = RIP;
               [out.RIPF,out.RIPFP,out.RIPFPar,out.RIPG,out.RIPGP,~] = SNPv4.performRIPStat(out.GG,RIP,var,0);
              ...
              while ContBoot && Bootstrap>0
                    ...
                    % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         if obj.Htest, M = H.*M;end% Only keep significant regression coefficients
                         rip = SNPv4.updateRIP(DepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = SNPv4.normalizeRIP(rip,M,var);end% rescale RIP scores
                         TMPIndVar(FiTestInd) = rip;% Store updated RIP scores, always in the last collumn
                     end
                   % monitoring progress of Booting
       ...
       function [M,H,Mfor] = robustPLSRegression(obj,indvar,depvar,gg,perc)
           ...
           % execute runs
             for s=1:1:runs
                 if balance % Balance the given input
                    if upsampling % Balancing by Increasing Minority Groups
                        ...
                        for i=1:1:nrgg
                            ...
                            [DepVarNew,IndVarNew] = SNPv4.regUpSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i});
                        ...
                        for i=1:1:nrgg
                            [DepVarNew,IndVarNew] = SNPv4.regDownSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i}.nrKeep);
                 ...
                 Mfor(s,:) = SNPv4.getRegression(SampleIndVar(SampleInd,:),SampleDepVar(SampleInd,:));
             end
           % finalizing output
             ...
             H = SNPv4.wilcoxonM(Mfor);% wilcoxon test for median
       end       
       ...
       function out = biometricEval(obj,GT,Faces,t)
                ...
                index = find(~isnan(gg));
                [out.Soft,out.Hard] = SNPv4.evalQD(obj.MOD.GG,obj.MOD.RIP,gg(index),rip(index)',obj.MOD.Var.PosClass,t);
       end
   end
   methods % MODEL EVALUATION
       function foldEvaluation(obj,t)
           ...
           for f=1:1:nrF
               [obj.EvalEER(f),obj.EvalG(f),obj.EvalR(f),obj.EvalpEER(f),obj.EvalpG(f),obj.EvalpR(f)] = SNPv4.evalQD(obj.Folds{f}.TrGG,obj.Folds{f}.TrRIP,obj.Folds{f}.GG,obj.Folds{f}.RIP,obj.MOD.Var.PosClass,t);
           end
           if t==0, return; end
           obj.EvalpEER(find(obj.EvalpEER==0)) = 0.1/t;
           obj.EvalpG(find(obj.EvalpG==0)) = 0.1/t;
           obj.EvalpR(find(obj.EvalpR==0)) = 0.1/t;
           obj.EvalFishEER = SNPv4.pfast(obj.EvalpEER);
           obj.EvalFishG = SNPv4.pfast(obj.EvalpG);
           obj.EvalFishR = SNPv4.pfast(obj.EvalpR);
       end  
   end
   methods % MODEL APPLICATION
       ...
       function [C,Atypicality,scan] = overLaySNPv4(obj,BaseCont,genotype,Ref,BF,maxAtyp,adjust,TrimBF)
                ...
                if GTInfo.Nr==0, return; end
                Xref = SNPv4.updateRIP(Ref.Coeff,obj.MOD.M);
                Xref = SNPv4.normalizeRIP(Xref,obj.MOD.M,obj.MOD.Var);
       ...
       function [IB,pTIB,pT] = matchFaces(obj,faces,genotype,BF)
                rip = SNPv4.updateRIP(faces,obj.MOD.M);
                rip = SNPv4.normalizeRIP(rip,obj.MOD.M,obj.MOD.Var);
                ...
                % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     if obj.MOD.GTInfo{i}.Nr>0
                        ...
                        [obj.MOD.GTInfo{i}.AVG,obj.MOD.GTInfo{i}.STD,obj.MOD.GTInfo{i}.MED,obj.MOD.GTInfo{i}.MAD,GTInfo{i}.O] = SNPv4.extractRobustRIPStatistics(obj.MOD.GTInfo{i}.RIP,kappa);                     
                     end
                 end
   ...
   methods (Static = true)
       ...
       function [F,FP,FPar,Gmean,GmeanP,GT] = performRIPStat(g,rip,var,t)
                ...
                [F,FPar] = SNPv4.ripAnova(XG,rip);
                if isnan(F), return; end % BAD TEST GROUP
                if doroc, [Gmean,GT] = SNPv4.ripROC(g,rip,var.PosClass); end
                if t == 0,return;end
                FCount = zeros(1,t);
                if doroc, GCount = zeros(1,t); end
                parfor i=1:t
                    ind = randperm(n);
                    Ffor = SNPv4.ripAnova(XG,rip(ind));
                    FCount(i) = Ffor>=F;
                    if doroc, Gmeanfor = SNPv4.ripROC(g,rip(ind),var.PosClass); GCount(i) = Gmeanfor>=Gmean; end
                end
                FP = sum(FCount)/t;
       ...
       function [Soft,Hard] = evalQD(trgg,trrip,gg,rip,posclass,t)
                ...
                [Soft.EER,Soft.G,Soft.AUC] = SNPv4.getEER(tmatches,fmatches);
                Soft.R = SNPv4.getRANK(tmatches,fmatches);
                % HARD ASSESSMENT
                [Hard.PREC,Hard.REC,Hard.G,Hard.TNF] = SNPv4.getHardClass(ratio,indexp,indexn,1);
                if t==0, return; end
                ...
                parfor i=1:t
                   ...
                   [forEER,~,forAUC] = SNPv4.getEER(fortmatches,forfmatches);
                   tmp = SNPv4.getRANK(fortmatches,forfmatches);
                   forR = tmp(4);
                   EERcount(i) = forEER<=Soft.EER;
                   AUCcount(i) = forAUC>=Soft.AUC;
                   Rcount(i) = forR<=Soft.R(4);
                   [~,~,forG] = SNPv4.getHardClass(forratio,indexp,indexn,1);
           ...
           % Obtaining group memberships and additional needed info
               ...
               % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     [out{i}.AVG,out{i}.STD,out{i}.MED,out{i}.MAD] = SNPv4.extractRIPStatistics(rip(out{i}.Ind));                     
                 end
...
%                 end
%                 [EER] = SNPv4.getEER(-log(ratio),-log(ratio.^-1));
%                 
%                 indexp = find(gg==posclass);
%                 [EERP] = SNPv4.getEER(-log(ratio(indexp)),-log(ratio(indexp).^-1));
%                 indexn = setdiff(1:n,indexp);
%                 [EERN] = SNPv4.getEER(-log(ratio(indexn)),-log(ratio(indexn).^-1));
%                 
%                 
%                 if t==0, return; end
%                 count = zeros(1,t);
...
%                    end
%                    [forEER] = SNPv4.getEER(-log(forratio),-log(forratio.^-1)); 
%                    count(i) = forEER<=EER;
%                 end
%                 P = sum(count)/t;
%        end

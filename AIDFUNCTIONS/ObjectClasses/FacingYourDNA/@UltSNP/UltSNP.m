classdef UltSNP < superClassSNP
   ...
   properties (Hidden = true)
      ...
      RIPNorm = true; % normalize RIP values based on group averages (SNPBRIM)
   ...
   methods % GETTING/SETTING
       ...
       function out = get.Balances(obj)
           ...
           out(1) = UltSNP.getBalance(obj.nAA+obj.nAB,obj.nBB);% Recessive
           out(2) = UltSNP.getBalance(obj.nBB+obj.nAB,obj.nAA);% Dominant
           out(3) = UltSNP.getBalance(obj.nAA+obj.nBB,obj.nAB);% Over Dominant
           out(4) = UltSNP.getBalance(obj.nAA,obj.nBB);% AA versus BB
           out(5) = UltSNP.getBalance(obj.nAA,obj.nAB);% AA versus AB
           out(6) = UltSNP.getBalance(obj.nAB,obj.nBB);% AB versus BB
       end
   ...
   methods % DATA CHECKING
       function out = dataCheck(obj,MinSampleSize)
           ...
           AAOk = obj.nAA>=MinSampleSize;
           ABOk = obj.nAB>=MinSampleSize;
           BBOk = obj.nBB>=MinSampleSize;
           switch obj.TestInd
               case 0
                   out = false;
       ...
       function out = dataCheckAf(obj,MinAlFreq)
           ...
           AAOk = obj.fAA>=MinAlFreq;
           ABOk = obj.fAB>=MinAlFreq;
           BBOk = obj.fBB>=MinAlFreq;
           switch obj.TestInd
               case 0
                   out = false;
               case {1 2 3 4} % All three groups required
   ...
   methods % SNP ANALYSIS
       function [LocD,LocP] = locationTest(obj,t,DepVar)
           ...
           if ~(obj.TestInd==4)
              [LocD,LocP] = UltSNP.DStatTest(DepVar(GG==-1,:),DepVar(GG==1,:),t); 
           else
              [LocD,LocP] = UltSNP.FStatTest(DepVar(GG==-1,:),DepVar(GG==0,:),DepVar(GG==1,:),t);
           end
       end
       function out = runR2Test(obj,t)
           [out.R2,out.pR2,out.M] = UltSNP.PartialPLSR(obj,t);
           obj.R2Test = out;
       end  
   end
            ...
            % Outer Partitioning of Data
              ...
              parfor fo=1:obj.OuterFold
                  ...
                  while ContBoot && Bootstrap>0
                   ...
                   % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         rip = UltSNP.updateRIP(FoTrDepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = UltSNP.normalizeRIP(rip,M,var);end% rescale RIP scores
                 ...
                 % Evaluate outer test data
                  ...
                  [FoldResults{fo}.F,FoldResults{fo}.FP,FoldResults{fo}.FPar,FoldResults{fo}.G,FoldResults{fo}.GP,~] = UltSNP.performRIPStat(FoldResults{fo}.GG,FoldResults{fo}.RIP,var,obj.RIPStatPerm);
             ...
             % Gathering Fold Results
               ...
               if ~isempty(index), out.FPar = UltSNP.pfast(FoldFPar); end
               out.FP = nan;
               out.GP = nan;
               if obj.RIPStatPerm>0
                  FoldFP(FoldFP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldFP));
                  if ~isempty(index), out.FP = UltSNP.pfast(FoldFP(index)); end
                  FoldGP(FoldGP==0) = 0.1/obj.RIPStatPerm;
                  index = find(~isnan(FoldGP));
                  if ~isempty(index), out.GP = UltSNP.pfast(FoldGP(index)); end
               end
               out.FoldF = FoldF;
               out.FoldG = FoldG;
              ...
              % Extracting final M 
               ...
               if obj.OuterFold>=8, 
                  out.H = UltSNP.wilcoxonM(M);
               else
                  out.H = ones(1,dim); 
               end
               out.M = out.H.*out.M;
               RIP = UltSNP.updateRIP(DepVar,out.M);
               if obj.RIPNormalize, RIP = UltSNP.normalizeRIP(RIP,out.M,var); end
               out.RIP = RIP;
               [out.RIPF,out.RIPFP,out.RIPFPar,out.RIPG,out.RIPGP,~] = UltSNP.performRIPStat(out.GG,RIP,var,0);
              ...
              while ContBoot && Bootstrap>0
                    ...
                    % seperate data into inner folds
                     ...
                     for fi=1:obj.InnerFold % Fi...
                         ...
                         if obj.Htest, M = H.*M;end% Only keep significant regression coefficients
                         rip = UltSNP.updateRIP(DepVar(FiTestInd,:),M)';% Get RIP scores
                         if obj.RIPNormalize, rip = UltSNP.normalizeRIP(rip,M,var);end% rescale RIP scores
                         TMPIndVar(FiTestInd) = rip;% Store updated RIP scores, always in the last collumn
                     end
                   % monitoring progress of Booting
       ...
       function [M,H,Mfor] = robustPLSRegression(obj,indvar,depvar,gg,perc)
           ...
           % execute runs
             for s=1:1:runs
                 if balance % Balance the given input
                    if upsampling % Balancing by Increasing Minority Groups
                        ...
                        for i=1:1:nrgg
                            ...
                            [DepVarNew,IndVarNew] = UltSNP.regUpSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i});
                        ...
                        for i=1:1:nrgg
                            [DepVarNew,IndVarNew] = UltSNP.regDownSample(depvar(balInfo{i}.Ind,:),indvar(balInfo{i}.Ind,:),balInfo{i}.nrKeep);
                 ...
                 Mfor(s,:) = UltSNP.getRegression(SampleIndVar(SampleInd,:),SampleDepVar(SampleInd,:));
             end
           % finalizing output
             ...
             H = UltSNP.wilcoxonM(Mfor);% wilcoxon test for median
       end       
   ...
   methods (Static = true)
       ...
       function [F,FP,FPar,Gmean,GmeanP,GT] = performRIPStat(g,rip,var,t)
                ...
                [F,FPar] = UltSNP.ripAnova(XG,rip);
                if isnan(F), return; end % BAD TEST GROUP
                if doroc, [Gmean,GT] = UltSNP.ripROC(g,rip,var.PosClass); end
                if t == 0,return;end
                FCount = zeros(1,t);
                if doroc, GCount = zeros(1,t); end
                parfor i=1:t
                    ind = randperm(n);
                    Ffor = UltSNP.ripAnova(XG,rip(ind));
                    FCount(i) = Ffor>=F;
                    if doroc, Gmeanfor = UltSNP.ripROC(g,rip(ind),var.PosClass); GCount(i) = Gmeanfor>=Gmean; end
                end
                FP = sum(FCount)/t;
       ...
       function [Soft,Hard] = evalQD(trgg,trrip,gg,rip,posclass,t)
                ...
                [Soft.EER,Soft.G,Soft.AUC] = UltSNP.getEER(tmatches,fmatches);
                Soft.R = UltSNP.getRANK(tmatches,fmatches);
                % HARD ASSESSMENT
                [Hard.PREC,Hard.REC,Hard.G,Hard.TNF] = UltSNP.getHardClass(ratio,indexp,indexn,1);
                if t==0, return; end
                ...
                parfor i=1:t
                   ...
                   [forEER,~,forAUC] = UltSNP.getEER(fortmatches,forfmatches);
                   tmp = UltSNP.getRANK(fortmatches,forfmatches);
                   forR = tmp(4);
                   EERcount(i) = forEER<=Soft.EER;
                   AUCcount(i) = forAUC>=Soft.AUC;
                   Rcount(i) = forR<=Soft.R(4);
                   [~,~,forG] = UltSNP.getHardClass(forratio,indexp,indexn,1);
           ...
           % Obtaining group memberships and additional needed info
               ...
               % Extracting group RIP Statistics  
                 for i=1:1:3
                     ...
                     [out{i}.AVG,out{i}.STD,out{i}.MED,out{i}.MAD] = UltSNP.extractRIPStatistics(rip(out{i}.Ind));                     
                 end
...
%                 end
%                 [EER] = UltSNP.getEER(-log(ratio),-log(ratio.^-1));
%                 
%                 indexp = find(gg==posclass);
%                 [EERP] = UltSNP.getEER(-log(ratio(indexp)),-log(ratio(indexp).^-1));
%                 indexn = setdiff(1:n,indexp);
%                 [EERN] = UltSNP.getEER(-log(ratio(indexn)),-log(ratio(indexn).^-1));
%                 
%                 
%                 if t==0, return; end
%                 count = zeros(1,t);
...
%                    end
%                    [forEER] = UltSNP.getEER(-log(forratio),-log(forratio.^-1)); 
%                    count(i) = forEER<=EER;
%                 end
%                 P = sum(count)/t;
%        end

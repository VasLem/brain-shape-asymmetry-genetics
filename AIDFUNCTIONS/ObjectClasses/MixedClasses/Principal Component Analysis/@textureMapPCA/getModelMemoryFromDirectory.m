function out = getModelMemoryFromDirectory(obj,filenames,rank,startN,nr)
...
% Created by Peter Claes
    ...
    n = length(filenames);
    f = waitbar(0,'Model f. Directory');drawnow;
    disp('Initial buildup');
    % Initial Model;
    [filenames,names] = selectFiles(filenames,startN);
    D = loadFiles(names,obj.nrP);
    [~,j] = find(isnan(D));
    j = unique(j);
    D(:,j) = repmat(obj.AvgVec,1,length(j));
    if obj.Centering, 
       [U,S,V] = svd(double(D)-repmat(obj.AvgVec,1,size(D,2)),'econ');
    else
       [U,S,V] = svd(double(D),'econ');
    end
    waitbar((n-length(filenames))/n,f);drawnow;
    r = min([size(U,1) rank size(V,2)]);
    U = U(:,1:r);
    S = diag(S(1:r,1:r)); 
    V = V(:,1:r);
    while ~isempty(filenames)
        [filenames,names] = selectFiles(filenames,nr);
        D = loadFiles(names,obj.nrP);
        [~,j] = find(isnan(D));
        j = unique(j);
        D(:,j) = repmat(obj.AvgVec,1,length(j));
        ...
        else % hier klopt iets niet zie ik nu even snsle zelf
            [U,S,V] = svd(double(D),'econ');
        end
        if size(U,2)>rank
           U = U(:,1:rank);
...
function [GF,GP] = selectFiles(G,nr)
         if nr < length(G);
             GP = G(:,1:nr);
             GF = G(:,nr+1:end);
         else
...
function [D] = loadFiles(names,nrP)
    D = nan*zeros(nrP,length(names));

classdef CPCA < superClass
    ...
    methods % Get & Set
        ...
        function out = get.nrVariables(obj)
            if isempty(obj.X), out = []; return; end;
            out = size(obj.X,2);
        end
        function updateBlockInfo(obj)
            if isempty(obj.BlockLabel), return; end
            obj.nrBlocks = length(unique(obj.BlockLabel));
            obj.BlockSizes = zeros(1,obj.nrBlocks);
            for i=1:1:obj.nrBlocks
                obj.BlockSizes(i) = sum(obj.BlockLabel==i);
            end
            return;
            
%             if isempty(obj.LandmarksLabel)
%                 obj.BlockLabel = (1:obj.nrVariables);
%                 obj.BlockLabel = repmat(obj.BlockLabel,3,1);
%                 obj.BlockLabel = reshape(obj.BlockLabel,[1 3*obj.nrVariables]);
%                 obj.nrBlocks = obj.nrVariables;
...
%             
%             obj.BlockLabel = zeros(1,3*obj.nrVariables);
%             for i=1:obj.nrVariables

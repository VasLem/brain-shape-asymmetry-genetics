function [ v ] = plotHierBrains3D(RefScan,label)
...
for n=1:size(circle_centers,2)
    ...
    clscan.Value = ones(1,clscan.nrV);
    rmindex = setdiff(1:scan.nrV,index);
    remscan = crop(scan,'VertexIndex',rmindex);
    remscan.Value = zeros(1,remscan.nrV);
    reduceVertices(remscan,750);

    clscan.Axes = v.RenderAxes;
    clscan.Visible = true;
    clscan.Selected = true;
    
    remscan.Axes = v.RenderAxes;
    remscan.ViewMode = 'Points';

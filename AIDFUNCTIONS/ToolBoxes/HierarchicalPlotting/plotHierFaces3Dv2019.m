function [ v ] = plotHierFaces3Dv2019(RefScan,label,colors)
...
FK = [0.8; 0.8; 0.8];
FK = repmat(FK,1,RefScan.nrV);
RefScan.TextureColor = FK;
...
for n=1:size(circle_centers,2)
    ...
    scan.TextureColor = FK;
    scan.TextureColor(:,index) = repmat(colors(:,n),1,length(index));

function myQQplot(pval,ax)
         if nargin<2, f = figure; ax = gca; end
         pval = pval(~isnan(pval));
         N = length(pval);
         u = rand(1,N);
         pval = -log10(sort(pval,'descend'));
         u = -log10(sort(u,'descend'));
         inqqplot(u,pval);
         axes(ax);
         axis square;
         xlabel('Expected -log(P)');ylabel('Observed -log(P)');
         axis square;
         r = max(max(u),max(pval));
         set(gca,'xlim',[0 max(u)],'ylim',[0 max(pval)]);
end

function h = inqqplot(x,y,pvec)
...
newplot();
hrefends = line(mx,my,'LineStyle','-.','Marker','none','LineWidth',2);
hrefmid = line(qx,qy,'LineStyle','-','Marker','none');
hdat = line(xx,yy,'LineStyle','none','Marker','+','MarkerSize',2);
if length(hdat)==1

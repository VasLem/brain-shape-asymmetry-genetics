function imageMorphPermAnalysis(analysis,RefScanV,vref,name,mi,pl,saveresults)
pval = 0.05;
%% imaging the scan
...
%% imaging the significance of the effect
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.pLocalS<=pval)','Indexed',[name ' Effect Sign.'],[0 1]);
...
%% imaging H1 pos of the displacements along the normals
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH1PosNormalDistances<=pval)','Indexed',[name ' Normal H1 +'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end
%% imaging H1 neg of the displacements along the normals
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH1NegNormalDistances<=pval)','Indexed',[name ' Normal H1 -'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end 
%% imaging H2 of the displacements along the normals
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH2NormalDistances<=pval)','Indexed',[name ' Normal H2'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end
...
%% imaging H1 pos Area Ratios
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH1PosAreaRatios<=pval)','Indexed',[name ' Area H1 +'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end
%% imaging H1 neg Area Ratios
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH1NegAreaRatios<=pval)','Indexed',[name ' Area H1 -'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end 
%% imaging H2 Area Ratios
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH2AreaRatios<=pval)','Indexed',[name ' Area H2'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end
...
%% imaging H1 pos signed curvature differences
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH1PosCurvDiff<=pval)','Indexed',[name ' Sign. Curv. H1 +'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end
%% imaging H1 neg signed curvature differences
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH1NegCurvDiff<=pval)','Indexed',[name ' Sign. Curv. H1 -'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end 
%% imaging H2 signed curvature differences
        [v,RefScan] = cloneAndDisplay(vref,RefScanV,'Value',double(analysis.STATS.comparison.pH2CurvDiff<=pval)','Indexed',[name ' Sign. Curv. H2'],[0 1]);
        colormap(v.RenderAxes,'summer');colorbar('peer',v.RenderAxes,'off');
        if saveresults,savingResults(v,RefScan);end             

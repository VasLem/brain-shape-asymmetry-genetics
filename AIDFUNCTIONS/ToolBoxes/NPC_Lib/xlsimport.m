...
%XLSIMPORT gets data and text from an Excel file ready to be used in NPClib.
%[D,data,code]=xlsimport(FILE,SHEET,RANGE,MODE) reads the data specified in
%RANGE from the worksheet SHEET, in the Excel file specified in FILE. 
%The data in the xls sheet are an n-by-m matrix (n number of observations and
%m variables) with variable names in the first row (accented 
%characters not accepted). See also example_xlsimport.xls in the
%DIR\NPClib\examples folder. D is a struct array containing the data and
%ready for use in NPClib. The fields of D are: vals, name and D. codedata
%is the n-by-m matrix of numerical data. code is the structure array
%containing recoding information for string obervations.
%

...
%tic search for a starting point
for i=1:nvert
    u=find(A(i,:)==1);
    if ~isempty(u)
        boundary=[i u(1)];
...
while(i<=nvert)
    ...
    diffu= setdiff(u,boundary);
    if isempty(diffu)       
       if isempty(memory), break; end
       diffu = memory(1);
    end
    s = diffu(1);
    if length(diffu)>1
       memory = union(memory,diffu(2:end));
    end
    boundary=[boundary s];
    memory = setdiff(memory,boundary);
    i=i+1;
    % take out 
end
toc
...
%         
%         
%     display('empty');keyboard; break; end;
%     s = diffu(1);
%     if length(diffu)>1
%        memory = setdiff(union(memory,diffu(2:end)),boundary);
%     end
%     if s~=boundary(1)
%         boundary=[boundary s];
%     else

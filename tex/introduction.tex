...
$$
The basic underlying assumptions is that each \ac{snp} effect is assessed independently from the rest and that the covariance matrix of the phenotype equals the identity matrix multiplied by N, namely that the studied traits are orthogonal to each other.

\subsection{\Ac{ldsc} genetic correlation}
\citet{Bulik-Sullivan2015_cor} also invented a way to utilize \ac{gwas} results produced for two different traits as a proxy to relate the genetic correlation of these traits, namely the extent over which the two characteristics are being regulated by similar genetic drivers:
$$
E[z_1z_2l_j] = \frac{\sqrt{N_1N_2}\rho_g}{M}l_j + \frac{\rho N_s}{\sqrt{N_1N_2}}
$$ 

...
\section{Neurogenesis, Neuronal Migration and Plasticity}
...
Although human cortex exhibits roughly symmetric structure, the symmetry is systematically suppressed, not only due to the environment, with plasticity playing central role, but also because of genetic factors, as explained in the previous sections. An asymmetric pattern is manifested across adult individuals, irrelevantly of their upbringing, comprising, therefore, a characteristic of the human species. A non-exhaustive list, with the most prominent asymmetric traits across healthy individuals, is the following \cite{Kuo2022}:
\begin{itemize}
	\item{Yakovlevian torque (\autoref{fig:yaktorque}): the left hemisphere prefrontal lobe and the right hemisphere occipital lobe tend to cross the symmetry plane, extending towards the other hemisphere, with these extensions. This creates a phenomenon of clockwise warping, making the whole brain appear slightly rotated, while making an impression on the inner part of the skull, called \textbf{petalia}.}
	\item{Peri-Sylvian asymmetry: the left Sylvian (lateral) fissure is longer and sharper than the right one, while the right Sylvian fissure exhibits a more visible leftward curve, in the part where temporal lobe meets the parietal lobe, that is the auditory cortex, also called \textbf{planum temporale}. The increased thickness of the right superior temporal lobe, that reduces the lateral fissure steepness, is attributed to increased white matter volume. Such trait has been reported to be gender-related, with males exhibiting greater asymmetry than females, as noted in previous studies, with steroid hormone receptor activity related genes, located in chromosome 6   \cite{Guadalupe2015}.}
	\item{Central sulcus asymmetry: the right hemisphere central sulcus is deeper and larger.}
	\item{}
\end{itemize}
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{Yakovlevian torque schematically illustrated (a), along with its manifestation in different axial section for a single individual (b,c) \cite{Kuo2022}.}
		\label{fig:yakovlevian_external}
	\end{subfigure}
\caption[Yakovlevian Torque \cite{Kuo2022}]{Illustration of the Yakovlevian torque. The angle of rotation in (A) was calculated by using the average angle of the longest edges, close to the midsagital plane, of the convex hulls of the horizontal plane projection of each hemisphere midthickness surface, after shape normalization (see \autoref{subsec:shape_normalization}), per individual, across the studied population.}
\label{fig:yaktorque}
\end{figure}
...
\subsection{Shapes Normalization}\label{subsec:shape_normalization}

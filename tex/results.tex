...
\begin{adjustwidth}{-3 cm}{-3 cm}\centering\begin{threeparttable}[!htb]
	...
	\begin{tabular}{ccc||cc||ccc||c||cc}\toprule
		...
		\bottomrule
	\end{tabular}
	\caption[Identified gene sets]{Identified gene sets using FUMA and GREAT for the entire hemisphere and the 2nd level partitioning \acp{gwas}. In bold, the genes that do not occur in other gene sets are displayed. Lead \acp{snp} that have not been mapped to a gene are also displayed with their rs-id.}
	\label{tab:genesets}
\end{threeparttable}\end{adjustwidth}
